...
def main():
    ...
    retired_portfolios = {r["uuid"] for r in conn.execute("SELECT uuid FROM portfolios WHERE is_retired=1").fetchall()}
    retired_accounts = {r["uuid"] for r in conn.execute("SELECT uuid FROM accounts WHERE is_retired=1").fetchall()}
    retired_securities = {r["uuid"] for r in conn.execute("SELECT uuid FROM securities WHERE retired=1").fetchall()}
    ...
    # Load Prices
    price_rows = conn.execute("SELECT date, security_uuid, close FROM historical_prices").fetchall()
    ...
    # Simulation State
    holdings = defaultdict(lambda: {"shares": 0.0, "cost_basis_eur": 0.0, "currency": "EUR"})
    ...
    sec_meta = {r["uuid"]: (r["currency_code"] or "EUR").strip().upper() for r in sec_rows}
    ...
    print(f"{'Date':<12} | {'Metric':<25} | {'DB Value':<15} | {'Calc Value':<15} | {'Diff':<10}")
    print("-" * 85)
    ...
    while current_date <= max_date:
        ...
        while tx_idx < len(txs) and parse_date(txs[tx_idx]["date"]) == current_date:
            ...
            if tx["portfolio"] in retired_portfolios: continue
            # if tx['account'] in retired_accounts: continue # FIXED: Holdings update still happens even if account is retired
            if tx["security"] and tx["security"] in retired_securities: continue
            ...
            # Explicit Types
            if t_type == 13: bucket_fees += abs(amount_eur) # Fee
            elif t_type == 14: bucket_fees -= abs(amount_eur) # Fee Refund
            elif t_type == 11: bucket_taxes += abs(amount_eur) # Tax
            elif t_type == 12: bucket_taxes -= abs(amount_eur) # Tax Refund
            elif t_type == 9: bucket_dividends += amount_eur # Dividend
            elif t_type == 8: bucket_interest += amount_eur # Interest
            elif t_type == 10: bucket_interest -= abs(amount_eur) # Interest Charge
            ...
            if t_type in (13, 14): classification_bucket = "fees"
            if t_type in (11, 12): classification_bucket = "taxes"

            if classification_bucket != "fees": bucket_fees += abs(fees_eur)
            if classification_bucket != "taxes": bucket_taxes += abs(taxes_eur)

            # Holdings Logic
            if sec_id:
                shares = (tx["shares"] or 0) / 100000000.0
                if t_type in (1, 3): shares *= -1 # Sign for Sell/Outbound
            ...
            if True:
                ...
                # Simplified:
                sign = 0
                if t_type in (6, 9, 8, 1, 14, 12): sign = 1 # Deposit, Div, Int, Sell, Refund
                elif t_type in (7, 0, 13, 11, 10): sign = -1 # Withdrawal, Buy, Fee, Tax, IntCharge
        ...
        if TARGET_START <= current_date <= TARGET_END:
            ...
            for sec_id, h in holdings.items():
                ...
                if date_iso == "2025-12-12":
                     print(f"DEBUG: Sec {sec_id} | Shares {h['shares']} | Price {price} | FX {s_fx} | Val {val_eur if s_fx > 0 else 0} | Currency {s_curr}")
            ...
            if db_rec:
                metrics = [
                    ...
                    ("Portfolio Wealth", db_rec["portfolio_wealth_eur"], total_portfolio_value),
                    ("Account Wealth", db_rec["account_wealth_eur"], account_cash_eur),
                    ("Invested Capital", db_rec["invested_capital_eur"], total_invested),
                    ("Realized Gains", db_rec["realized_gains_eur"], daily_realized),
                    ("Unrealized Gains", db_rec["unrealized_gains_eur"], calc_unrealized),
                ...
                for label, db_val, calc_val in metrics:
                    ...
                    if abs(db_val) < 0.01 and abs(calc_val) < 0.01: continue

                    diff = abs(db_val - calc_val)
                    warn = " <--- MISMATCH" if diff > 0.05 else ""
                    print(f"{date_iso:<12} | {label:<25} | {db_val:<15.2f} | {calc_val:<15.2f} | {diff:<10.2f}{warn}")

        current_date += datetime.timedelta(days=1)

    # Final Holdings Verification
    print("\n--- Final Holdings Verification ---")
    db_holdings = conn.execute("SELECT security_uuid, SUM(current_holdings) as shares_int FROM portfolio_securities GROUP BY security_uuid").fetchall()
    db_shares_map = {r["security_uuid"]: r["shares_int"] / 100000000.0 for r in db_holdings}
    ...
    for sec in all_secs:
        ...
        if abs(calc_s - db_s) > 0.0001:
            print(f"Mismatch Sec {sec}: Calc {calc_s:.4f} | DB {db_s:.4f} | Diff {calc_s - db_s:.4f}")
            # Get name
            name_row = conn.execute("SELECT name FROM securities WHERE uuid=?", (sec,)).fetchone()
