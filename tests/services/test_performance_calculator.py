...
def _insert_tx(conn, uuid, type, date_str, security, shares_norm, amount_norm, currency, account="acc1"):
...
def _insert_price(conn, security, date_obj, price_norm):
    ...
    # date stored as unix days (engine_pandas convention) or just check what we implemented.
    # In implementation: pd.to_datetime(df_prices["date"], unit="D", origin="unix")
    # So we must store days since epoch.
...
def test_standard_share_b_example(conn, calc):
    ...
    # Need prices for dates? The logic uses Transaction Amount if present.
    # Logic: if amount != 0, price = amount / shares.
    # Buy: 150/1 = 150. Sell: 160/1 = 160.
    # FX is EUR (1.0).
    # Baseline for Sell (in period) is Buy Price (since bought in period).
    # Gain = (160 * 1) - (150 * 1) = 10.
...
def test_unrealized_gain_held_at_start(conn, calc):
    ...
    # Price at Start (2023-01-01)
    # Using previous day close usually?
    # Logic: get_price(sec_id, start_date).
    # If explicit price exists on start date, it uses it.
    _insert_price(conn, "sec1", start_date, 110.0)

    # Price at End (2023-01-31)
...
def test_realized_gain_held_at_start(conn, calc):
    ...
    # Price at End for remaining 5 shares
...
def test_fx_performance_cash(conn, calc):
    ...
    # FX Rate at Deposit (irrelevant for period start, but good for tracking)
    _insert_fx(conn, "2022-12-01", "USD", 0.90) # 1 USD = 0.9 EUR

    # FX Rate at Start
    _insert_fx(conn, "2023-01-01", "USD", 0.95)

    # Transaction during period: Buy something for 500 USD
...
def test_absolute_performance(conn, calc):
    ...
    # We need to populate daily_wealth
