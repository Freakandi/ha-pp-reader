...
export class DateRangePicker {
  ...
  private activeDropdownTrigger: HTMLElement | null = null; // Element that triggered the dropdown
  ...
  private closeDropdown() {
    ...
    if (this.activeDropdownTrigger && this.activeDropdownTrigger.isConnected) {
  ...
  private renderCalendar(date: Date, position: "left" | "right") {
    ...
    monthBtn.onclick = (e) => {
      ...
      const allHeaders =
        monthBtn
          .closest(".drp-calendar-header")
          ?.querySelectorAll(".drp-header-btn") ?? [];
      ...
      const isNowOpen =
        this.activeDropdown && monthContainer.contains(this.activeDropdown);
    ...
    yearBtn.onclick = (e) => {
      ...
      const isNowOpen =
        this.activeDropdown && yearContainer.contains(this.activeDropdown);
    ...
    for (let d = 1; d <= lastDay.getDate(); d++) {
      ...
      // Labels and A11y
      let label = current.toLocaleDateString("de-DE", {
        weekday: "long",
        day: "numeric",
        month: "long",
        year: "numeric",
  ...
  private toggleMonthDropdown(
    ...
    this.closeDropdown();
    this.activeDropdownTrigger = trigger;
