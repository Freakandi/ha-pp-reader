...
function renderCoverageBadges(records: DailyWealthRecord[]): string {
  ...
  const fxIssue = records.some(
    (record) =>
      record.fx_coverage_ratio != null && record.fx_coverage_ratio < 1,
  );
  const priceIssue = records.some(
    (record) =>
      record.price_coverage_ratio != null && record.price_coverage_ratio < 1,
  );
  ...
  if (fxIssue) {
    badges.push(
      '<span class="meta-badge meta-badge--warning" title="Wechselkurse fehlen teilweise">FX-Abdeckung</span>',
    );
  }
  if (priceIssue) {
    badges.push(
      '<span class="meta-badge meta-badge--warning" title="Preisdaten unvollständig">Preisabdeckung</span>',
    );
  }
  if (staleIssue) {
    badges.push(
      '<span class="meta-badge meta-badge--neutral" title="Letzte Kurse sind veraltet">Stale Kurse</span>',
    );
  }
...
function setStatus(
  card: HTMLElement,
  status: DailyWealthState["status"],
  message = "",
): void {
...
function renderTotals(
  ...
  const selectionEl = card.querySelector<HTMLElement>(
    "#analyse-selection-label",
  );
...
function renderMetrics(
  ...
  if (!records.length) {
    container.innerHTML =
      '<div class="metrics-empty">Keine Daten verfügbar</div>';
    return;
  }
      ...
      if (expanded) {
        ...
        try {
          ...
          loadingRow.innerHTML =
            '<span class="metric-label">Lade Details...</span><span class="metric-value">...</span>';
          ...
          const response =
            await hass.connection.sendMessagePromise<BreakdownResponse>({
          ...
          if (!items || items.length === 0) {
            ...
            emptyRow.innerHTML =
              '<span class="metric-label">Keine Details</span><span class="metric-value">—</span>';
          ...
          errRow.innerHTML =
            '<span class="metric-label">Fehler beim Laden</span>';
...
function buildScopeKey(
  type: DailyWealthScopeRecord["scope_type"] | null,
  id: string | null,
): string | null {
  if (!type || !id) {
    return null;
...
function renderScopeFilters(
  ...
  if (!slices || (!hasAccounts && !hasPortfolios)) {
    container.innerHTML =
      '<p class="table-note" role="note"><span class="table-note__icon" aria-hidden="true">ℹ️</span><span>Keine Slices verfügbar.</span></p>';
    return;
  }
  ...
  container.addEventListener("change", (event) => {
    ...
    )?.closest<HTMLInputElement>('input[type="checkbox"][data-scope-key]');
...
function buildSeries(data: DailyWealthResponse): WealthSeries[] {
        ...
        .map((point, index) => {
          ...
          const ratioX =
            range.maxX === range.minX
              ? 0.5
              : (timestamp - range.minX) / (range.maxX - range.minX);
          const ratioY =
            range.maxY === range.minY
              ? 0.5
              : (point.value - range.minY) / (range.maxY - range.minY);
...
function renderWealthChart(
  ...
  const container = chartCard.querySelector<HTMLElement>(
  ...
  let chartHost: ReturnType<typeof renderLineChart> | null =
    existing as unknown as ReturnType<typeof renderLineChart> | null;
  ...
  if (!existing.__chartState || !container.querySelector("svg")) {
    ...
    updateLineChart(
      existing as unknown as ReturnType<typeof renderLineChart>,
      chartOptions,
    );
...
function derivePerformance(
  ...
  const netTransfers =
    sumField(periodRecords, "inbound_transfers_eur") -
    sumField(periodRecords, "outbound_transfers_eur");
  ...
  const uPriceStart =
    records.length > 1 ? (baseline.unrealized_price_gains_eur ?? 0) : 0;
...
function initRangeCard(
  ...
  const pickerContainer = card.querySelector<HTMLElement>(
    "#analyse-date-picker-container",
  );

  const currentSelection =
    lastSelection ?? getDailyWealthState().selection ?? buildDefaultSelection();
