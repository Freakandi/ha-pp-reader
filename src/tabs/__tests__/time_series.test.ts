...
test("Analyse tab renders totals, performance, coverage badges, and chart slices", () => {
  ...
  const headline =
    root.querySelector("#analyse-total-wealth")?.textContent ?? "";
  ...
  const scopeCheckbox = root.querySelector<HTMLInputElement>(
    'input[data-scope-key="account:acc-1"]',
  );
  assert.ok(scopeCheckbox);
  scopeCheckbox.checked = false;
  scopeCheckbox.dispatchEvent(
...
test("derivePerformanceForTest reconciles totals and cashflows", () => {
  ...
  const delta =
    ...
    breakdown.neutral;
  assert.ok(
    Math.abs(delta - 200) < 1e-6,
    "components should sum to total change",
  );
});
