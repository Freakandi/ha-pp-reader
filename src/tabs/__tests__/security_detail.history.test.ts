...
test("resolveRangeOptionsForTest returns epoch-day bounds for range queries", () => {
  ...
  assert.strictEqual(
    options.end_date,
    19725,
    "end_date should follow epoch-day encoding",
  );
  assert.strictEqual(
    options.start_date,
    19696,
    "start_date should track the 30-day window in epoch days",
  );
});
...
test("normaliseTransactionMarkersForTest filters, maps types, and formats labels", () => {
  ...
  assert.strictEqual(
    markers.length,
    3,
    "only buy/sell/delivery markers with dates should be included",
  );
  ...
  assert.match(
    buyMarker.label ?? "",
    /Kauf 20\b/,
    "buy label should include share count",
  );
  assert.match(
    buyMarker.label ?? "",
    /120,43 USD/,
  ...
  assert.match(
    sellMarker.label ?? "",
    /Verkauf 5,50 @ 150,50 USD/,
    "sell label should include price with fallback currency",
  );
  assert.match(
    sellMarker.label ?? "",
    /\(netto 148,25 EUR\)/,
    "sell label should append net EUR price when available",
  );
  ...
  assert.match(
    outboundMarker.label ?? "",
    /Verkauf/,
    "delivery type 3 should map to sale label",
  );
});
...
test("transaction marker tooltips show type, shares, date, and currency", () => {
  ...
  try {
    ...
    const xFormatted = new Date("2025-01-03T00:00:00Z").toLocaleDateString(
      "de-DE",
    );
...
void test("purchase markers are rendered after switching history ranges", async () => {
  ...
  (
    globalThis as typeof globalThis & {
      requestAnimationFrame?: typeof env.window.requestAnimationFrame;
    }
  ).requestAnimationFrame = env.window.requestAnimationFrame.bind(env.window);
  (
    globalThis as typeof globalThis & {
      cancelAnimationFrame?: typeof env.window.cancelAnimationFrame;
    }
  ).cancelAnimationFrame = env.window.cancelAnimationFrame.bind(env.window);
  ...
  try {
    const hassStub = {
      ...
      connection: {
        ...
        sendMessagePromise: (payload: Record<string, unknown>) => {
          ...
          throw new Error(
            `Unexpected WebSocket payload: ${String(payload.type)}`,
          );
        },
      },
    ...
    const rangeButton = root.querySelector<HTMLButtonElement>(
    ...
    const chartContainer = root.querySelector<HTMLDivElement>(
      ".line-chart-container",
    );
    assert.ok(chartContainer, "expected chart container after range change");
    type ChartState = { markers?: { label?: string }[] };
    const markers = (
      chartContainer as HTMLDivElement & { __chartState?: ChartState }
    ).__chartState?.markers;
    assert.ok(Array.isArray(markers), "chart state markers should be an array");
    assert.strictEqual(
      markers.length,
      1,
      "chart should retain purchase marker after range switch",
    );
    assert.match(
      markers[0].label ?? "",
      /Kauf/,
      "marker label should describe purchase",
    );
  } finally {
