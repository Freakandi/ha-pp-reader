...
test("composeAveragePurchaseTooltipForTest renders FX information from averages", () => {
  ...
  assert.ok(
    tooltip,
    "expected tooltip to be generated when averages are available",
  );
  const tooltipText = tooltip;
  assert.ok(
    tooltipText,
    "tooltip text should be non-empty when averages exist",
  );
...
test("composeAveragePurchaseTooltipForTest falls back to EUR when account currency matches security", () => {
  ...
  assert.ok(
    tooltip,
    "expected tooltip even when account currency matches security",
  );
  assert.match(
    tooltip,
    /1 HKD = 0,1105 EUR/,
    "tooltip should reflect EUR fallback rate",
  );
});
...
test("resolvePurchaseFxTimestampForTest prefers dedicated FX timestamps", () => {
  ...
  assert.ok(
    fallbackTimestamp,
    "expected fallback timestamp when dedicated value is absent",
  );
  assert.strictEqual(
...
test("buildHeaderMetaForTest renders EUR average when provided alongside native", () => {
  ...
  assert.match(
    averageText,
    /17,77\s*EUR/,
    "EUR average should be shown alongside native",
  );
});
