...
function renderTradesTable(trades: readonly RealizedTrade[]): string {
  ...
  const cols = [
    ...
    {
      key: "total_sold_shares",
      label: createSimpleSortHeader("Stück", "total_sold_shares"),
      align: "right" as const,
    },
    {
      key: "last_sell_date",
      label: createSimpleSortHeader("Datum", "last_sell_date"),
      align: "right" as const,
    },
    ...
    {
      key: "result_pct",
      label: createSimpleSortHeader("Resultat", "result_pct"),
      align: "right" as const,
    },
    {
  ...
  const rows: TradesTableRow[] = trades.map((trade) => {
    ...
    const priceTrend =
      priceDiff > 0 ? "positive" : priceDiff < 0 ? "negative" : "neutral";
  ...
  // --- Footer Calculation ---
  const totalPurchase = trades.reduce(
    (sum, t) => sum + t.purchase_value_gross,
    0,
  );
  const totalSales = trades.reduce((sum, t) => sum + t.sales_value_gross, 0);
  const totalResultGross = trades.reduce(
    (sum, t) => sum + (t.sales_value_gross - t.purchase_value_gross),
    0,
  );
  const totalResultNet = trades.reduce((sum, t) => sum + t.result_abs, 0);
  const totalSinceSellAbs = trades.reduce(
    (sum, t) => sum + t.since_sell_abs,
    0,
  );

  // Percentage of total result over total purchase value
  const totalResultPct =
    totalPurchase !== 0 ? (totalResultGross / totalPurchase) * 100 : 0;
...
function sortTrades(
  ...
  tbody.querySelectorAll("tr.child-row").forEach((tr) => {
    tr.remove();
  });
...
function attachEventListeners(
  ...
  const handleInteraction = (event: Event) => {
    ...
    if (expandIcon) {
      ...
      const mainRow = root.querySelector(
        `tr[data-security-uuid="${String(uuid)}"]`,
      );
      ...
      if (mainRow && trade) {
        ...
        if (isExpanded) {
          mainRow
            .querySelector(".expand-icon ha-icon")
            ?.setAttribute("icon", "mdi:chevron-down");
          ...
          const childRows = Array.from(
            tempDiv.querySelectorAll("tbody tr"),
          ).map((row) => {
          ...
          mainRow
            .querySelector(".expand-icon ha-icon")
            ?.setAttribute("icon", "mdi:chevron-right");
          root
            .querySelectorAll(
              `tr.child-row[data-parent-uuid="${String(uuid)}"]`,
            )
            .forEach((child) => {
              child.remove();
            });
