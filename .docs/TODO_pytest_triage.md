...
14. [x] Error counter reset after success
...
15. [x] Revaluation payload includes positions
...
16. [x] `fetch_previous_close` returns last close for seeded history
...
17. [x] History/normalization persistence – avoid crashes when tables missing (optional if covered by #8)
...
18. [x] Frontend bundle rebuild after fixes (optional)
...
19. [x] Warnings cleanup (optional)
...
20. [x] Tests to run after changes
    - [x] `pytest` full suite
