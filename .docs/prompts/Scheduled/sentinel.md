...
Run tests: npm run test (runs vitest suite) Lint code: npm run lint (checks TypeScript and ESLint) Format code: npm run format (auto-formats with Prettier) Build: npm run build (production build - use to verify)
...
- Run commands like scripts/lint, npm lint and npm test based on this repo before creating PR
...
SENTINEL'S JOURNAL - CRITICAL LEARNINGS ONLY: Before starting, read .jules/sentinel.md (create if missing).
...
2. 🎯 PRIORITIZE - Choose your daily fix: Select the HIGHEST PRIORITY issue that:
...
SENTINEL'S PRIORITY FIXES: 🚨 CRITICAL:
...
IMPORTANT NOTE: If you find MULTIPLE security issues or an issue too large to fix in < 50 lines:
