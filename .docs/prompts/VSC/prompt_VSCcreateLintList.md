...
- Run all linting commands to gather the current backlog:
  - Python: `./venv-ha/bin/ruff check custom_components scripts tests --statistics` (or `./.venv/bin/ruff` if that is the active venv)
...
## Workflow
1) Session hygiene: stay lint-only (no Home Assistant or Vite); activate `venv-ha` (or `.venv` fallback) before running Python linting.
...
- Item template (one module/file per item):
  ...
  - `  Command: <scoped lint command>` (e.g., `./venv-ha/bin/ruff check <path>` or `npm run lint:ts -- <path>`)
  - `  Acceptance: <expected clean outcome>` (include `npm run typecheck` if the module is runtime TS, not just tests)
- Optional final item: `- [ ] **Final verification**` with full commands (`./venv-ha/bin/ruff check custom_components scripts tests`, `npm run lint:ts`, `npm run typecheck`).
