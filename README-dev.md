...
- `data/coordinator.PPReaderCoordinator` caches the latest normalization result per config entry after each metrics run (feature flag `normalized_pipeline`) and pushes incremental updates via `data/event_push._push_update`. Events share the canonical `data_type` values (`accounts`, `portfolio_values`, `portfolio_positions`, `security_snapshot`, `security_history`) used by the WebSocket handlers.
- Diagnostics expose the cached normalization result as `normalized_payload` when the feature flag is enabled; use that entry to compare database state with the panel or CLI responses before debugging deeper layers.
