const cn = /* @__PURE__ */ new Set(), ln = /* @__PURE__ */ new Set(), fr = {}, Ja = [
...
function ai(e) {
  if (!_e(e))
    return null;
  const t = e.aggregation, n = O(e.security_uuid), r = O(e.name), a = V(e.current_holdings), i = V(e.purchase_value_eur) ?? (_e(t) ? V(t.purchase_value_eur) ?? V(t.purchase_total_account) ?? V(t.account_currency_total) : null) ?? V(e.purchase_value), o = V(e.current_value);
  ...
  const c = {
    ...
    purchase_value: i,
    current_value: o,
    average_cost: rt(e.average_cost),
    performance: rt(e.performance),
    aggregation: rt(e.aggregation),
    data_state: pr(e.data_state)
  }, s = V(e.coverage_ratio);
  s != null && (c.coverage_ratio = s);
  const l = O(e.provenance);
  l && (c.provenance = l);
  const f = tt(e.metric_run_uuid);
...
function yr(e) {
  ...
  const a = V(
    ...
    metric_run_uuid: tt(e.metric_run_uuid) ?? void 0,
    data_state: pr(e.data_state)
  };
...
function br(e) {
  if (!_e(e))
    return null;
  const t = { ...e }, n = tt(e.metric_run_uuid);
...
function vr(e) {
  ...
  if (!t)
    return null;
  const n = tt(e.metric_run_uuid), r = hr(e.accounts), a = _r(e.portfolios), i = ii(e.diagnostics), o = {
    generated_at: t,
    metric_run_uuid: n,
    accounts: r,
    portfolios: a
...
function ci(e) {
  const t = xn(e.security_uuid, "security_uuid"), n = xn(e.name, "name"), r = $t(e.current_holdings, "current_holdings"), a = $t(e.purchase_value, "purchase_value"), i = $t(e.current_value, "current_value"), o = {
    security_uuid: t,
    name: n,
    current_holdings: r,
    purchase_value: a,
...
async function di(e, t) {
  ...
  const r = await e.connection.sendMessagePromise({
    ...
    entry_id: n
  }), a = _r(r.portfolios), i = vr(r.normalized_payload);
  return {
    portfolios: a,
    normalized_payload: i
  };
...
function Fn(e) {
  const t = Sr(e), n = ne(e.scope_type), r = ne(e.scope_id);
...
async function gi(e, t, n) {
  ...
  o !== void 0 && (u.include_slices = o), c !== void 0 && (u.include_scopes = c), (Array.isArray(s?.accounts) || Array.isArray(s?.portfolios)) && (u.scopes = {}, Array.isArray(s.accounts) && (u.scopes.accounts = s.accounts.filter((_) => typeof _ == "string" && _.length > 0)), Array.isArray(s.portfolios) && (u.scopes.portfolios = s.portfolios.filter(
  ...
  const g = await e.connection.sendMessagePromise(u), y = fi(g.range, d, p), m = (Array.isArray(g.records) ? g.records : []).map((_) => _ && typeof _ == "object" ? Sr(_) : null).filter((_) => !!_), b = pi(g.slices);
  return {
    range: y,
    records: m,
    ...b ? { slices: b } : {}
...
function Ar(e, t) {
  return `<span class="trend--${e > 0 ? "positive" : e < 0 ? "negative" : "neutral"}">${t}</span>`;
}
const dn = (e, t) => {
...
function Ee(e, t, n = [], r = {}) {
  ...
  t.forEach((h) => {
    ...
    if (a && h.key) {
      const b = `${me(h.label)} sortieren`;
      s += `<th${m} data-sort-key="${h.key}" role="button" tabindex="0" aria-sort="none" aria-label="${b}">${h.label}</th>`;
    } else
      s += `<th${m}>${h.label}</th>`;
  }), s += "</tr></thead><tbody>", e.forEach((h) => {
    s += "<tr>", t.forEach((m) => {
      const b = m.align === "right" ? ' class="align-right"' : "";
      s += `<td${b}>${M(m.key, h[m.key], h)}</td>`;
    }), s += "</tr>";
  });
  const l = {}, f = {};
  t.forEach((h) => {
  ...
  if (u != null && (g = `${ae(u)} %`, u > 0 ? y = "positive" : u < 0 && (y = "negative")), s += '<tr class="footer-row">', t.forEach((h, m) => {
    ...
    if (l[h.key] != null) {
      let v = "";
      h.key === "gain_abs" && g && (v = ` data-gain-pct="${me(g)}" data-gain-sign="${me(y)}"`), s += `<td${b}${v}>${M(h.key, l[h.key], void 0, f[h.key])}</td>`;
      return;
    }
    if (h.key === "gain_pct" && l.gain_pct != null) {
      s += `<td${b}>${M("gain_pct", l.gain_pct, void 0, f[h.key])}</td>`;
      return;
    }
    const _ = f[h.key] ?? { hasValue: !1 };
    s += `<td${b}>${M(h.key, null, void 0, _)}</td>`;
...
function Cr(e, t, n = "asc", r = !1) {
  ...
  o.sort((f, d) => {
    ...
    let b;
    const _ = /[0-9]/.test(g) || /[0-9]/.test(y);
    return !Number.isNaN(h) && !Number.isNaN(m) && _ ? b = h - m : b = g.localeCompare(y, "de", { sensitivity: "base" }), n === "asc" ? b : -b;
...
function Nt(e, { decimals: t = Si, fallback: n = null } = {}) {
  const r = he(e);
  if (r == null)
    return n ?? null;
...
function Ci(e) {
  ...
  if (n == null && r == null && a == null && i == null)
    return null;
  const o = Pr(t.source) ?? "derived", c = ce(t.coverage_ratio) ?? null;
  return {
...
function Ae(e) {
  ...
  if (n == null || r == null || a == null || i == null)
    return null;
  const o = Pr(t.source) ?? "derived", c = ce(t.coverage_ratio) ?? null, s = Ci(t.day_change);
  return {
    gain_abs: n,
    gain_pct: r,
    total_change_eur: a,
...
function Ni(e, t) {
  ...
  return o !== void 0 && (n.performance = Ei(c, o, [
    "gain_pct",
    "total_change_pct"
...
function ki(e) {
  if (!e || typeof e != "object")
    return null;
  const t = Pi(e) ? Le(e) : e, n = be(t.security_uuid), r = be(t.name), a = he(t.current_holdings), i = $n(t.current_value), o = pn(t.aggregation), c = t.aggregation && typeof t.aggregation == "object" ? t.aggregation : null, s = q(t.purchase_value_eur) ?? q(c?.purchase_value_eur) ?? q(c?.purchase_total_account) ?? q(c?.account_currency_total) ?? $n(t.purchase_value);
...
function xr(e) {
  ...
  const c = Pe(e.day_change_pct);
  c !== void 0 && (n.day_change_pct = c);
  const s = Tn(e.position_count);
  ...
  const f = Pe(e.coverage_ratio);
  f !== void 0 && (n.coverage_ratio = f);
  const d = at(e.provenance);
...
function Mi(e) {
  ...
  for (const n of t) {
    ...
    if (!r)
      continue;
    const a = we.get(r.uuid), i = a ? Ti(a, r) : gn(r);
    we.set(i.uuid, i);
  }
}
function ut(e, t) {
...
function Hi() {
  return Array.from(we.values(), (e) => gn(e));
}
function kr() {
  return {
    accounts: Ri(),
...
function Vi(e) {
  ...
  if (!t.startsWith("{") && !t.startsWith("["))
    return null;
  try {
    const n = JSON.parse(t), r = Ui(n), a = n && typeof n == "object" ? te(
      n.provider ?? n.source
    ) : null;
    if (r.length && a)
      return `${Tr(a)} (${r.join(", ")})`;
...
function qi(e) {
  if (!e)
    return null;
  const t = te(e.uuid) ?? `${Ii}-${e.name ?? "0"}`, n = Fr(e.name, "Unbenanntes Konto"), r = te(e.currency_code), a = J(e.balance), i = J(e.orig_balance), o = "coverage_ratio" in e ? $r(J(e.coverage_ratio)) : null, c = te(e.provenance), s = te(e.metric_run_uuid), l = e.fx_unavailable === !0, f = J(e.fx_rate), d = te(e.fx_rate_source), p = te(e.fx_rate_timestamp), u = [], g = Rr(c);
  g && u.push(g);
  const y = {
    uuid: t,
    name: n,
...
function Wi(e) {
  ...
  if (!t)
    return null;
  const n = Fr(e.name, "Unbenanntes Depot"), r = Rn(e.position_count), a = Rn(e.missing_value_positions), i = J(e.current_value), o = J(e.purchase_sum) ?? J(e.purchase_value_eur) ?? J(e.purchase_value) ?? 0, c = J(e.day_change_abs) ?? null, s = J(e.day_change_pct) ?? null, l = Ae(e.performance), f = l?.gain_abs ?? null, d = l?.gain_pct ?? null, p = l?.day_change ?? null;
  ...
  const y = i != null, h = e.has_current_value === !1 || !y, m = "coverage_ratio" in e ? $r(J(e.coverage_ratio)) : null, b = te(e.provenance), _ = te(e.metric_run_uuid), v = [], A = Rr(b);
  A && v.push(A);
  const C = {
    uuid: t,
    name: n,
...
function Lr() {
  const { accounts: e } = kr();
  return e.map(qi).filter((t) => !!t);
}
function Bi() {
  const { portfolios: e } = kr();
  return e.map(Wi).filter((t) => !!t);
}
function Mr(e, t = {}) {
  if (!e || e.length === 0)
    return "";
...
const Ki = 500, Gi = 10, Xi = "pp-reader:portfolio-positions-updated", Zi = "pp-reader:diagnostics", Tt = /* @__PURE__ */ new Map(), Ir = [
  "coverage_ratio",
  "provenance",
  "metric_run_uuid",
  "generated_at"
...
function io(e, t) {
  return `<div class="error">${k(Oi(e))} <button class="retry-pos" data-portfolio="${t}">Erneut laden</button></div>`;
}
function oo(e, t, n) {
...
function zr(e, t, n, r) {
  ...
  if (r)
    return i.innerHTML = io(r, t), { applied: !0 };
  const o = i.dataset.sortKey, c = i.dataset.sortDir;
  return i.innerHTML = Ur(n), o && (i.dataset.sortKey = o), c && (i.dataset.sortDir = c), oo(i, e, t), { applied: !0 };
}
function yn(e, t) {
  const n = ue.get(t);
  if (!n) return !1;
...
function Vr(e, t) {
  ...
  n.timer || (n.timer = setTimeout(() => {
    ...
    const r = yn(e, t);
    r || n.attempts >= Gi ? (Ye.delete(t), r || ue.delete(t)) : Vr(e, t);
  }, Ki), Ye.set(t, n));
}
function co(e, t) {
  ...
  const a = t.querySelector(".portfolio-table table"), i = a ? Array.from(
    ...
    const l = o.cells.item(3), f = it(l?.textContent);
    return {
...
function lo(e, t) {
  ...
  if (r) {
    const o = i.map((c) => {
      const s = c.orig_balance, l = typeof s == "number" && Number.isFinite(s), f = ze(c.currency_code), d = l ? s.toLocaleString("de-DE", {
...
function fo(e, t) {
  ...
  for (const [d, p] of f.entries()) {
    ...
    const g = u.cells.item(1), y = u.cells.item(2), h = u.cells.item(3), m = u.cells.item(4), b = u.cells.item(5), _ = u.cells.item(6), v = u.cells.item(7);
    if (!g || !y || !h)
      continue;
    const A = typeof p.position_count == "number" && Number.isFinite(p.position_count) ? p.position_count : 0, C = typeof p.current_value == "number" && Number.isFinite(p.current_value) ? p.current_value : null, P = Ae(p.performance), N = typeof P?.gain_abs == "number" ? P.gain_abs : null, F = typeof P?.gain_pct == "number" ? P.gain_pct : null, I = typeof p.purchase_sum == "number" && Number.isFinite(p.purchase_sum) ? p.purchase_sum : typeof p.purchase_value == "number" && Number.isFinite(p.purchase_value) ? p.purchase_value : null, w = P?.day_change ?? null, x = De(p.day_change_abs) ?? De(w?.value_change_eur) ?? De(w?.price_change_eur), U = De(p.day_change_pct) ?? De(w?.change_pct);
    let E = x ?? null, $ = U ?? null;
    ...
    const K = typeof p.missing_value_positions == "number" && Number.isFinite(p.missing_value_positions) ? p.missing_value_positions : 0, S = C !== null, D = p.has_current_value === !1 || K > 0 || !S, L = it(h.textContent);
    it(g.textContent) !== A && (g.textContent = l(A));
    const T = {
      fx_unavailable: D,
      current_value: C,
      performance: P
    ...
    const Y = M("current_value", T.current_value, T, z), X = typeof C == "number" ? C : 0;
    if ((Math.abs(L - X) >= 5e-3 || h.innerHTML !== Y) && (h.innerHTML = Y, u.classList.add("flash-update"), setTimeout(() => {
      u.classList.remove("flash-update");
    }, 800)), m && (m.innerHTML = M("day_change_abs", E, T, z)), b && (b.innerHTML = M("day_change_pct", $, T, z)), _) {
      const G = M("gain_abs", N, T, z);
      _.innerHTML = G;
  ...
  try {
    const d = (...m) => {
      ...
      if (!m) return [];
      const _ = m.querySelectorAll("tbody tr.account-row");
      return (_.length ? Array.from(_) : Array.from(m.querySelectorAll("tbody tr:not(.footer-row)"))).map((A) => {
        const C = b ? A.cells.item(2) : A.cells.item(1);
        return { balance: it(C?.textContent) };
      });
    }, y = [
      ...g(p, !1),
      ...g(u, !0)
      ...
      const b = m.dataset.currentValue, _ = m.dataset.purchaseSum, v = b ? Number.parseFloat(b) : Number.NaN, A = _ ? Number.parseFloat(_) : Number.NaN;
      return {
...
function zn(e, t) {
  ...
  if (d.applied) {
    ...
    r || d.reason !== "hidden" || l ? (ue.set(n, { positions: f, error: r }), Vr(t, n)) : (ue.delete(n), Ye.delete(n));
  if (!r && o.length > 0) {
    const p = Array.from(
      new Set(
    ...
    if (o) {
      ...
      if (f)
        ...
        o.querySelectorAll("tbody tr").forEach((p, u) => {
          ...
          if (!g)
            return;
          const y = e[u], h = Mn(y), m = typeof h?.gain_pct == "number" && Number.isFinite(h.gain_pct) ? h.gain_pct : null, b = m != null ? `${m.toLocaleString("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          })} %` : "—", _ = m == null ? "neutral" : m > 0 ? "positive" : m < 0 ? "negative" : "neutral";
...
function mo(e) {
  ...
  const n = Array.from(e.querySelectorAll("tbody tr.portfolio-row")), r = (m) => {
    ...
    (m, b) => {
      ...
      if (_ != null && (m.sumPositions += _), b.dataset.fxUnavailable === "true" && (m.fxUnavailable = !0), b.dataset.hasValue !== "true")
        return m.incompleteRows += 1, m;
      m.valueRows += 1;
      const v = r(b.dataset.currentValue), A = r(b.dataset.gainAbs), C = r(b.dataset.purchaseSum);
  ...
  h && (h.dataset.gainPct = i && typeof o == "number" ? `${jt(o)} %` : "—", h.dataset.gainSign = i && typeof o == "number" ? o > 0 ? "positive" : o < 0 ? "negative" : "neutral" : "neutral"), c.dataset.positionCount = Math.round(a.sumPositions).toString(), c.dataset.currentValue = i ? a.sumCurrent.toString() : "", c.dataset.purchaseSum = i ? a.sumPurchase.toString() : "", c.dataset.gainAbs = i ? a.sumGainAbs.toString() : "", c.dataset.gainPct = i && typeof o == "number" ? o.toString() : "", c.dataset.hasValue = i ? "true" : "false", c.dataset.fxUnavailable = a.fxUnavailable || !i ? "true" : "false";
}
function Vn(e) {
  if (typeof e == "number" && Number.isFinite(e))
    return e;
...
function qr(e, t, n) {
  ...
  f ? f.textContent = `${jt(s)} €` : l.textContent = `💰 Gesamtvermögen: ${jt(s)} €`, l.dataset.totalWealthEur = s.toString();
}
function yo(e, t) {
...
const Tc = {
  getPortfolioPositionsCacheSnapshot: Di,
...
function wo(e) {
  const t = e, n = e.average_cost ?? null, r = e.aggregation ?? null, a = On(t, [
    ...
    a === "EUR" ? "EUR" : null
  ) ?? "EUR", o = qe(n?.native), c = qe(n?.security), s = qe(n?.account), l = qe(n?.eur), f = c ?? o, d = l ?? (i === "EUR" ? s : null), p = a ?? i, u = p === "EUR";
  let g, y;
  u ? (g = "EUR", y = d ?? f ?? s ?? null) : f != null ? (g = p, y = f) : s != null ? (g = i, y = s) : (g = "EUR", y = d ?? null);
  const h = Yn(y, g), m = u ? null : Yn(d, "EUR"), b = !!m && m !== h, _ = [], v = [];
  h ? (_.push(
    `<span class="purchase-price purchase-price--primary">${h}</span>`
  ), v.push(h.replace(/\u00A0/g, " "))) : (_.push('<span class="missing-value" role="note" aria-label="Kein Kaufpreis verfügbar" title="Kein Kaufpreis verfügbar">—</span>'), v.push("Kein Kaufpreis verfügbar")), b && m && (_.push(
    `<span class="purchase-price purchase-price--secondary">${m}</span>`
  ), v.push(m.replace(/\u00A0/g, " ")));
  const A = _.join("<br>"), C = qe(r?.purchase_value_eur) ?? 0, P = v.join(", ");
  return { markup: A, sortValue: C, ariaLabel: P };
}
function Ao(e) {
  const t = he(e.current_holdings);
  if (t == null)
    return { value: null, pct: null };
  const n = he(e.last_price_eur), r = he(e.last_close_eur);
  let a = null, i = null;
  if (n != null && r != null) {
    a = (n - r) * t;
    const d = r * t;
...
function Xe(e) {
  ...
  const n = [
    ...
    const o = Ae(i.performance), c = typeof o?.gain_abs == "number" ? o.gain_abs : null, s = typeof o?.gain_pct == "number" ? o.gain_pct : null, l = Ao(i), f = typeof i.purchase_value == "number" || typeof i.purchase_value == "string" ? i.purchase_value : null;
    return {
  ...
  }), a = Ee(r, n, ["purchase_value", "current_value", "day_change_abs", "gain_abs"]);
  try {
    const i = document.createElement("template");
    i.innerHTML = a.trim();
    const o = i.content.querySelector("table");
    if (o) {
      ...
      return n.forEach((l, f) => {
        ...
        if (u) {
          const { markup: h, sortValue: m, ariaLabel: b } = wo(d);
          u.innerHTML = h, u.dataset.sortValue = String(m), b ? u.setAttribute("aria-label", b) : u.removeAttribute("aria-label");
        }
        const g = l.cells.item(7);
        if (g) {
          const h = Ae(d.performance), m = typeof h?.gain_pct == "number" && Number.isFinite(h.gain_pct) ? h.gain_pct : null, b = m != null ? `${m.toLocaleString("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          })} %` : "—", _ = m == null ? "neutral" : m > 0 ? "positive" : m < 0 ? "negative" : "neutral";
          g.dataset.gainPct = b, g.dataset.gainSign = _;
...
function Po(e, t) {
  ...
  r && (r.__ppReaderSecurityClickBound || (r.__ppReaderSecurityClickBound = !0, r.addEventListener("click", (a) => {
    ...
    if (s)
      try {
        ka(s) || console.warn("attachSecurityDetailDelegation: Detail-Tab konnte nicht geöffnet werden für", s);
      } catch (l) {
...
function Or(e) {
  ...
  r.forEach((S) => {
    ...
    const D = Number.isFinite(S.position_count) ? S.position_count : 0, L = Number.isFinite(S.purchase_sum) ? S.purchase_sum : 0, j = S.hasValue && typeof S.current_value == "number" && Number.isFinite(S.current_value) ? S.current_value : null, T = j !== null, z = S.performance, B = typeof S.gain_abs == "number" ? S.gain_abs : typeof z?.gain_abs == "number" ? z.gain_abs : null, Y = typeof S.gain_pct == "number" ? S.gain_pct : typeof z?.gain_pct == "number" ? z.gain_pct : null, X = z && typeof z == "object" ? z.day_change : null, G = typeof S.day_change_abs == "number" ? S.day_change_abs : X && typeof X == "object" ? X.value_change_eur ?? X.price_change_eur : null, Ve = typeof S.day_change_pct == "number" ? S.day_change_pct : X && typeof X == "object" && typeof X.change_pct == "number" ? X.change_pct : null, Ce = S.fx_unavailable && T, $a = typeof S.coverage_ratio == "number" && Number.isFinite(S.coverage_ratio) ? S.coverage_ratio : "", Ta = typeof S.provenance == "string" ? S.provenance : "", Ra = typeof S.metric_run_uuid == "string" ? S.metric_run_uuid : "", Ue = gt.has(S.uuid), La = Ue ? "portfolio-toggle expanded" : "portfolio-toggle", Pn = `portfolio-details-${S.uuid}`, Q = {
      ...
      gain_abs: B,
      gain_pct: Y
    }, xe = { hasValue: T }, Ma = M("purchase_value", Q.purchase_value, Q, xe), Ha = M("current_value", Q.current_value, Q, xe), Ia = M("day_change_abs", Q.day_change_abs, Q, xe), za = M("day_change_pct", Q.day_change_pct, Q, xe), Va = M("gain_abs", Q.gain_abs, Q, xe), Ua = M("gain_pct", Q.gain_pct, Q, xe), En = T && typeof Y == "number" && Number.isFinite(Y) ? `${ae(Y)} %` : "", qa = T && typeof Y == "number" && Number.isFinite(Y) ? Y > 0 ? "positive" : Y < 0 ? "negative" : "neutral" : "", Wa = T && typeof j == "number" && Number.isFinite(j) ? j : "", Ba = T && typeof B == "number" && Number.isFinite(B) ? B : "", Oa = T && typeof Y == "number" && Number.isFinite(Y) ? Y : "", Ya = T && typeof G == "number" && Number.isFinite(G) ? G : "", ja = T && typeof Ve == "number" && Number.isFinite(Ve) ? Ve : "", Ka = String(D);
    let Ft = "";
    En && (Ft = ` data-gain-pct="${t(En)}" data-gain-sign="${t(qa)}"`), Ce && (Ft += ' data-partial="true"'), n += `<tr class="portfolio-row"
                  data-portfolio="${S.uuid}"
                  data-position-count="${Ka}"
                  data-current-value="${t(Wa)}"
                  data-purchase-sum="${t(L)}"
                  data-day-change="${t(Ya)}"
                  data-day-change-pct="${t(ja)}"
                  data-gain-abs="${t(Ba)}"
                data-gain-pct="${t(Oa)}"
                data-has-value="${T ? "true" : "false"}"
                data-fx-unavailable="${S.fx_unavailable ? "true" : "false"}"
                data-coverage-ratio="${t($a)}"
                data-provenance="${t(Ta)}"
                data-metric-run-uuid="${t(Ra)}">`;
    const Ga = k(S.name), Xa = Mr(Wr(S.badges), {
    ...
    const Za = D.toLocaleString("de-DE");
    n += `<td class="align-right">${Za}</td>`, n += `<td class="align-right">${Ma}</td>`, n += `<td class="align-right">${Ha}</td>`, n += `<td class="align-right">${Ia}</td>`, n += `<td class="align-right">${za}</td>`, n += `<td class="align-right"${Ft}>${Va}</td>`, n += `<td class="align-right gain-pct-cell">${Ua}</td>`, n += "</tr>", n += `<tr class="portfolio-details${Ue ? "" : " hidden"}"
                data-portfolio="${S.uuid}"
                id="${Pn}"
                role="region"
                aria-label="Positionen für ${S.name}">
      <td colspan="${r.length.toString()}">
        <div class="positions-container">${Ue ? xt(S.uuid) ? Xe(Er(S.uuid)) : fn("Lade Positionen...") : ""}</div>
      </td>
    </tr>`;
  });
  const a = e.filter((S) => typeof S.current_value == "number" && Number.isFinite(S.current_value)), i = e.reduce((S, D) => S + (Number.isFinite(D.position_count) ? D.position_count : 0), 0), o = a.reduce((S, D) => typeof D.current_value == "number" && Number.isFinite(D.current_value) ? S + D.current_value : S, 0), c = a.reduce((S, D) => typeof D.purchase_sum == "number" && Number.isFinite(D.purchase_sum) ? S + D.purchase_sum : S, 0), s = a.map((S) => {
    ...
    gain_abs: d ? f : null,
    gain_pct: d ? y : null
  }, m = { hasValue: d }, b = { hasValue: u }, _ = M("purchase_value", h.purchase_value, h, m), v = M("current_value", h.current_value, h, m), A = M("day_change_abs", h.day_change_abs, h, b), C = M("day_change_pct", h.day_change_pct, h, b), P = M("gain_abs", h.gain_abs, h, m), N = M("gain_pct", h.gain_pct, h, m);
  let F = "";
  ...
  const I = String(Math.round(i)), w = d ? String(o) : "", x = d ? String(c) : "", U = u ? String(l) : "", E = u && typeof g == "number" && Number.isFinite(g) ? String(g) : "", $ = d ? String(f) : "", K = d && typeof y == "number" && Number.isFinite(y) ? String(y) : "";
...
function Yr(e) {
  ...
  for (const L of r) {
    const j = We(L.dataset.positionCount);
    ...
    l = !0;
    const B = We(L.dataset.currentValue), Y = We(L.dataset.gainAbs), X = We(L.dataset.purchaseSum), G = We(L.dataset.dayChange);
  ...
  const m = Math.round(a).toLocaleString("de-DE"), b = {
    fx_unavailable: p || !u,
    ...
    gain_abs: u ? c : null,
    gain_pct: u ? g : null
  }, _ = { hasValue: u }, v = { hasValue: f && u }, A = M("purchase_value", b.purchase_value, b, _), C = M("current_value", b.current_value, b, _), P = M("day_change_abs", b.day_change_abs, b, v), N = M("day_change_pct", b.day_change_pct, b, v), F = M("gain_abs", b.gain_abs, b, _), I = M("gain_pct", b.gain_pct, b, _), w = t.tHead ? t.tHead.rows.item(0) : null, x = w ? w.cells.length : 0, U = h.cells.length, E = x || U, $ = E > 0 ? E <= 5 : !1, K = u && typeof g == "number" ? `${ae(g)} %` : "", S = u && typeof g == "number" ? g > 0 ? "positive" : g < 0 ? "negative" : "neutral" : "neutral";
  $ ? h.innerHTML = `
      <td>Summe</td>
...
function Je(e, t) {
  ...
  const i = (u, g) => {
    ...
    if (!y) return;
    const h = Array.from(y.querySelectorAll("tr")).filter((v) => !v.classList.contains("footer-row")), m = y.querySelector("tr.footer-row"), b = (v) => {
      if (v == null) return 0;
    ...
    const _ = a.querySelector(`thead th[data-sort-key="${u}"]`);
    _ && (_.classList.add("sort-active", g === "asc" ? "dir-asc" : "dir-desc"), _.setAttribute("aria-sort", g === "asc" ? "ascending" : "descending")), h.forEach((v) => y.appendChild(v)), m && y.appendChild(m);
  }, o = r.dataset.sortKey, c = r.dataset.sortDir, s = a.dataset.defaultSort, l = a.dataset.defaultDir, f = Lt(o) ? o : Lt(s) ? s : "name", d = Mt(c) ? c : Mt(l) ? l : "asc";
  i(f, d);
  const p = (u) => {
    const g = u.target;
    ...
    if (!Lt(h))
      return;
    let m = "asc";
    r.dataset.sortKey === h && (m = (Mt(r.dataset.sortDir) ? r.dataset.sortDir : "asc") === "asc" ? "desc" : "asc"), r.dataset.sortKey = h, r.dataset.sortDir = m, i(h, m);
  };
...
async function No(e, t, n) {
  ...
  if (!(a && a.classList.contains("hidden"))) {
    ...
    try {
      ...
      lt(e, o), ut(e, o), r.innerHTML = Xe(o);
      try {
        Je(n, e);
      } catch (c) {
        ...
        (async () => {
          try {
            ...
            if (f.classList.contains("hidden")) {
              f.classList.remove("hidden"), s.classList.add("expanded"), s.setAttribute("aria-expanded", "true"), d && (d.textContent = "▼"), gt.add(l);
              try {
                yn(e, l);
              } catch (u) {
              ...
              f.classList.add("hidden"), s.classList.remove("expanded"), s.setAttribute("aria-expanded", "false"), d && (d.textContent = "▶"), gt.delete(l);
          } catch (o) {
...
async function jr(e, t, n) {
  ft = t ?? null, pt = n ?? null, console.debug(
    "renderDashboard: start – panelConfig:",
    n?.config,
    "derived entry_id?",
    n?.config?._panel_custom?.config?.entry_id
  );
  const r = await li(t, n);
  Dr(r.accounts);
  const a = Lr(), i = await di(t, n);
  Li(i.portfolios);
  const o = Bi();
  let c = "";
  try {
  ...
  `, b = Et("Übersicht", m), _ = Or(o), v = a.filter((w) => (w.currency_code ?? "EUR") === "EUR"), A = a.filter((w) => (w.currency_code ?? "EUR") !== "EUR"), P = A.some((w) => w.fx_unavailable) ? `
...
function ko(e, t) {
  ...
  const n = () => {
    try {
      ...
      i && i.querySelectorAll(".portfolio-toggle").length === 0 && (console.debug("Recovery: Tabelle ohne Buttons – erneuter Aufbau"), i.innerHTML = Or(t)), _n(e), Do(e), gt.forEach((o) => {
        try {
          xt(o) && (Je(e, o), Ze(e, o));
        } catch (c) {
...
const Fo = "http://www.w3.org/2000/svg", ke = 640, Fe = 260, Be = { top: 12, right: 16, bottom: 24, left: 16 }, Oe = "var(--pp-reader-chart-line, #3f51b5)", Kt = "var(--pp-reader-chart-area, rgba(63, 81, 181, 0.12))", jn = "0.75rem", Kr = "var(--pp-reader-chart-baseline, rgba(96, 125, 139, 0.75))", Gr = "6 4", $o = 1440 * 60 * 1e3;
function To(e) {
  if (e == null)
    return null;
  if (typeof e == "string")
...
const Zr = (e) => {
  ...
  if (t && typeof t == "object" && "date" in t) {
    const r = t.date, a = Ro(r);
    if (a)
      return a;
  }
  return Number.isFinite(e) ? e.toString() : "";
}, ea = (e, t, n) => (Number.isFinite(e) ? e : ht(e, 0) ?? 0).toLocaleString("de-DE", {
  minimumFractionDigits: 2,
  maximumFractionDigits: 2
...
function Ho(e) {
  ...
  if (!t)
    return;
  const r = n?.color ?? Kr, a = n?.dashArray ?? Gr;
...
function Ht(e) {
  ...
  const { minY: c, maxY: s, boundedHeight: l } = r, f = Number.isFinite(c) ? c : o, p = (Number.isFinite(s) ? s : f + 1) - f, u = p === 0 ? 0.5 : (o - f) / p, g = re(u, 0, 1), y = Math.max(l, 0), h = a.top + (1 - g) * y, m = Math.max(i - a.left - a.right, 0), b = a.left, _ = a.left + m;
  t.setAttribute("x1", b.toFixed(2)), t.setAttribute("x2", _.toFixed(2)), t.setAttribute("y1", h.toFixed(2)), t.setAttribute("y2", h.toFixed(2)), t.style.opacity = "1";
}
function Io(e, t, n) {
  ...
  const s = e.map((E, $) => {
    const K = o(E, $), S = c(E, $), D = Xr(K, $), L = ht(S, Number.NaN);
  ...
  if (s.length === 0)
    return { points: [], range: null };
  const l = s.reduce((E, $) => Math.min(E, $.xValue), s[0].xValue), f = s.reduce((E, $) => Math.max(E, $.xValue), s[0].xValue), d = s.reduce((E, $) => Math.min(E, $.yValue), s[0].yValue), p = s.reduce((E, $) => Math.max(E, $.yValue), s[0].yValue), u = Math.max(r - i.left - i.right, 1), g = Math.max(a - i.top - i.bottom, 1), y = Number.isFinite(l) ? l : 0, h = Number.isFinite(f) ? f : y + 1, m = Number.isFinite(d) ? d : 0, b = Number.isFinite(p) ? p : m + 1, _ = ht(t.baseline?.value, null), v = _ != null && Number.isFinite(_) ? Math.min(m, _) : m, A = _ != null && Number.isFinite(_) ? Math.max(b, _) : b, C = Math.max(
    2,
    Math.min(
      6,
      Math.round(
  ...
  ), { niceMin: P, niceMax: N } = Oo(
    v,
    A,
    C
  ), F = Number.isFinite(P) ? P : m, I = Number.isFinite(N) ? N : b, w = h - y || 1, x = I - F || 1;
  return {
    points: s.map((E) => {
      const $ = w === 0 ? 0.5 : (E.xValue - y) / w, K = x === 0 ? 0.5 : (E.yValue - F) / x, S = i.left + $ * u, D = i.top + (1 - K) * g;
      return {
        ...E,
        x: S,
        y: D
...
function It(e) {
  ...
  r.forEach((l, f) => {
    const d = Xr(l.x, f), p = ht(l.y, Number.NaN), u = Number(p);
    if (!Number.isFinite(d) || !Number.isFinite(u))
      return;
    const g = c === 0 ? 0.5 : re((d - a.minX) / c, 0, 1), y = s === 0 ? 0.5 : re((u - a.minY) / s, 0, 1), h = i.left + g * a.boundedWidth, m = i.top + (1 - y) * a.boundedHeight, b = oe("g", {
...
function aa(e, t, n, r) {
  e.width = Number.isFinite(t) ? Number(t) : ke, e.height = Number.isFinite(n) ? Number(n) : Fe, e.margin = {
...
function Vo(e, t, n, r = null) {
  ...
  const d = a.offsetWidth || 0, p = a.offsetHeight || 0, u = t.x * s, g = re(
    ...
    o.left * s,
    (i - o.right) * s - d
  ), y = Math.max(f * l - p, 0), h = 12, b = (Number.isFinite(n) ? re(n ?? 0, o.top, f) : t.y) * l;
  let _ = b - p - h;
  _ < o.top * l && (_ = b + h), _ = re(_, 0, y);
  const v = ee(Math.round(g)), A = ee(Math.round(_));
...
function qo(e, t, n, r = null) {
  ...
  const p = a.offsetWidth || 0, u = a.offsetHeight || 0, g = t.x * l, y = re(
    ...
    o.left * l,
    (i - o.right) * l - p
  ), h = Math.max(d * f - u, 0), m = 10, b = s?.getBoundingClientRect(), _ = e.svg?.getBoundingClientRect(), v = b && _ ? b.top - _.top : null, A = b && _ ? b.bottom - _.top : null, P = (Number.isFinite(n) ? re(n ?? t.y, o.top, d) : t.y) * f;
  let N;
...
function Bo(e, t) {
  ...
  const n = (a) => {
    ...
    const i = t.svg.getBoundingClientRect(), o = t.width || ke, c = t.height || Fe, s = i.width && Number.isFinite(i.width) && Number.isFinite(o) && o > 0 ? i.width / o : 1, l = i.height && Number.isFinite(i.height) && Number.isFinite(c) && c > 0 ? i.height / c : 1, f = s > 0 ? 1 / s : 1, d = l > 0 ? 1 / l : 1, p = (a.clientX - i.left) * f, u = (a.clientY - i.top) * d, g = {
      scaleX: s,
      scaleY: l
    };
    ...
    ), t.focusLine.style.opacity = "1"), t.tooltip && (t.tooltip.innerHTML = zo(t, y), Vo(t, y, u, g));
    const m = Wo(t, p, u);
    m && t.markerTooltip ? (t.markerTooltip.innerHTML = Uo(t, m), qo(t, m, u, g)) : ot(t);
  }, r = () => {
    Gt(t), ot(t);
  };
...
function ia(e, t = {}) {
  ...
  const r = oe("svg", {
    width: ke,
    height: Fe,
    viewBox: `0 0 ${String(ke)} ${String(Fe)}`,
    role: "img",
    "aria-hidden": "true",
    focusable: "false"
  });
  ...
  const g = ra(n);
  if (g.svg = r, g.areaPath = a, g.linePath = o, g.baselineLine = i, g.focusLine = c, g.focusCircle = s, g.overlay = f, g.tooltip = d, g.markerOverlay = p, g.markerLayer = l, g.markerTooltip = u, g.xAccessor = t.xAccessor ?? Zr, g.yAccessor = t.yAccessor ?? Jr, g.xFormatter = t.xFormatter ?? Qr, g.yFormatter = t.yFormatter ?? ea, g.tooltipRenderer = t.tooltipRenderer ?? ta, g.markerTooltipRenderer = t.markerTooltipRenderer ?? na, g.color = t.color ?? Oe, g.areaColor = t.areaColor ?? Kt, g.baseline = t.baseline ?? null, g.handlersAttached = !1, g.markers = Array.isArray(t.markers) ? t.markers.slice() : [], !g.xAxis) {
  ...
  return aa(g, t.width, t.height, t.margin), o.setAttribute("stroke", g.color), c.setAttribute("stroke", g.color), s.setAttribute("stroke", g.color), a.setAttribute("fill", g.areaColor), bn(n, t), Bo(n, g), n;
}
function bn(e, t = {}) {
  if (!e) {
  ...
  const { points: i, range: o } = Io(n.series, n, {
    xAccessor: n.xAccessor,
    yAccessor: n.yAccessor
  });
  if (n.points = i, n.range = o, i.length === 0) {
    n.linePath.setAttribute("d", ""), n.areaPath && n.areaPath.setAttribute("d", ""), Gt(n), It(n), zt(n), Ht(n);
    return;
  }
  if (i.length === 1) {
    ...
    n.linePath.setAttribute("d", f), n.areaPath && n.areaPath.setAttribute("d", ""), n.focusCircle && (n.focusCircle.setAttribute("cx", s.x.toFixed(2)), n.focusCircle.setAttribute("cy", s.y.toFixed(2)), n.focusCircle.style.opacity = "1"), n.focusLine && (n.focusLine.style.opacity = "0"), zt(n), Ht(n), It(n);
    return;
  }
  const c = Mo(i);
  if (n.linePath.setAttribute("d", c), n.areaPath && o) {
    const s = n.margin.top + o.boundedHeight, l = Lo(i, s);
...
function zt(e) {
  ...
  if (t.style.left = ee(a.left), t.style.width = ee(y), t.style.top = ee(i - a.bottom + 6), t.innerHTML = "", u && y > 0) {
    const b = (s - c) / $o, _ = Math.max(2, Math.min(6, Math.round(y / 140) || 4));
    Yo(e, c, s, _, b).forEach(({ positionRatio: A, label: C }) => {
      const P = document.createElement("div");
  ...
  if (n.style.left = "0", n.style.width = ee(Math.max(m, 0)), n.innerHTML = "", g && h > 0) {
    const b = Math.max(2, Math.min(6, Math.round(h / 60) || 4)), _ = jo(l, f, b), v = o;
    _.forEach(({ value: A, positionRatio: C }) => {
      const P = document.createElement("div");
...
function Oo(e, t, n = 4) {
  ...
  if (t === e) {
    const l = Xt(Math.abs(e) || 1);
    return {
      niceMin: e - l,
      niceMax: t + l
    };
  }
  const i = (t - e) / (r - 1), o = Xt(i), c = Math.floor(e / o) * o, s = Math.ceil(t / o) * o;
  return c === s ? {
    niceMin: e,
    niceMax: t + o
  } : {
...
function jo(e, t, n) {
  ...
  const r = t - e, a = Math.max(2, n), i = r / (a - 1), o = Xt(i), c = Math.floor(e / o) * o, s = Math.ceil(t / o) * o, l = [];
...
function Zo(e) {
  return e instanceof CustomEvent ? Xo(e.detail) : !1;
}
const Vt = { min: 0, max: 6 }, mt = { min: 2, max: 4 }, Jo = "1Y", oa = [
  "1M",
  "6M",
  "1Y",
  "5Y",
...
}, es = /* @__PURE__ */ new Set([0, 2]), ts = /* @__PURE__ */ new Set([1, 3]), ns = "var(--pp-reader-chart-marker-buy, #2e7d32)", rs = "var(--pp-reader-chart-marker-sell, #c0392b)", Gn = "{TICKER}", as = "https://chatgpt.com/", Ut = {
  aggregation: "Aggregationsdaten",
  totals: "Kaufsummen",
  eur_total: "EUR-Kaufsumme"
}, $e = /* @__PURE__ */ new Map(), st = /* @__PURE__ */ new Map(), Qe = /* @__PURE__ */ new Map(), Te = /* @__PURE__ */ new Map(), sa = "pp-reader:portfolio-positions-updated", je = /* @__PURE__ */ new Map();
function is(e) {
...
function cs(e, t) {
  ...
  (Array.isArray(n) ? n : []).includes(e) && (ua(e), da(e));
}
function ls(e) {
  if (!e || je.has(e))
    return;
  ...
  try {
    window.addEventListener(sa, t), je.set(e, t);
  } catch (n) {
...
function us(e) {
  ...
  const t = je.get(e);
  try {
    t && window.removeEventListener(sa, t);
  } catch (n) {
...
function fa(e) {
  return st.get(e)?.activeRange ?? Jo;
}
function Zt(e) {
  const t = Date.UTC(
    e.getUTCFullYear(),
...
function Zn(e) {
  return !(e instanceof Date) || Number.isNaN(e.getTime()) ? null : Zt(Me(e));
}
function H(e) {
  return he(e);
}
...
function ga(e, t = "Unbekannter Fehler") {
  if (typeof e == "string") {
    const n = e.trim();
    return n || t;
  }
...
function yt(e, t) {
  const n = Me(t instanceof Date ? t : /* @__PURE__ */ new Date()), r = Qo[e], a = Zn(n), i = {};
...
function bt(e, t, n = null) {
  if (!Array.isArray(e))
    return [];
  const r = [], a = Ne(t), i = a || "EUR", o = fs(n);
  return e.forEach((c, s) => {
    const l = typeof c.type == "number" ? c.type : Number(c.type), f = es.has(l), d = ts.has(l);
    if (!f && !d)
      return;
    const p = ps(c.date);
    let u = H(c.price);
    if (!p || u == null)
      return;
    const g = Ne(c.currency_code), y = a ?? g ?? i;
    g && a && g !== a && se(o) && (u *= o);
    const h = H(c.shares), m = H(c.net_price_eur), b = f ? "Kauf" : "Verkauf", _ = h != null ? `${An(h)} @ ` : "", v = `${b} ${_}${ye(u)} ${y}`, A = d && m != null ? `${v} (netto ${ye(m)} EUR)` : v, C = f ? ns : rs, P = typeof c.uuid == "string" && c.uuid.trim() || `${b}-${p.getTime().toString()}-${s.toString()}`;
    r.push({
      id: P,
      x: p.getTime(),
      y: u,
...
function gs(e) {
  if (!e)
    return null;
  const n = e.last_price_fetched_at, r = _t(n);
  if (r != null)
    return r;
...
function Qt(e, t) {
  ...
  if (!R(a))
    return r;
  const i = gs(t) ?? Date.now(), o = new Date(i);
...
function ha(e, t) {
  ...
  if (!R(o))
    return { priceChange: null, priceChangePct: null };
  const c = o - r, s = Object.is(c, -0) ? 0 : c, l = hs(o, r);
...
function ma(e, t, n, r) {
  ...
  return `
    <div class="security-info-bar" data-range="${me(a)}">
...
function An(e) {
  ...
  if (t == null)
    return "—";
  const n = Math.abs(t % 1) > 0, r = n ? 2 : Vt.min, a = n ? Vt.max : Vt.min;
...
function ye(e) {
  ...
  return t == null ? "—" : t.toLocaleString("de-DE", {
    minimumFractionDigits: mt.min,
    maximumFractionDigits: mt.max
  });
}
function bs(e, t) {
  const n = ye(e), r = `&nbsp;${k(t)}`;
  return `<span class="${wn(e, mt.max)}">${n}${r}</span>`;
}
function vs(e, t) {
...
function Cs(e, t, n) {
  const r = Ie(e?.average_cost), a = r?.account ?? (R(t) ? t : H(t));
  if (!R(a))
    return null;
  const i = e?.account_currency_code ?? e?.account_currency;
  if (typeof i == "string" && i.trim())
    return i.trim().toUpperCase();
  const o = Ne(e?.currency_code) ?? "", c = r?.security ?? r?.native ?? (R(n) ? n : H(n)), s = pn(e?.aggregation);
  if (o && R(c) && Ke(a, c))
    return o;
...
function Ns(e, t) {
  if (!e)
    return null;
  const n = Ne(e.currency_code) ?? "", r = Ie(e.average_cost);
  if (!r || !n)
    return null;
  const a = r.native ?? r.security ?? null;
  let o = r.account ?? r.eur ?? null, c = Ne(t) ?? "";
  ...
  if (s > 0) {
    const b = 1 / s;
    Number.isFinite(b) && b > 0 && (f = Jn(b));
  }
  const d = Ps(e), p = Es(d), u = [`FX-Kurs (Kauf): 1 ${n} = ${l} ${c}`];
  f && u.push(`1 ${c} = ${f} ${n}`);
  const g = [], y = r.source, h = y in Ut ? Ut[y] : Ut.aggregation;
...
function xs(e) {
  ...
  const t = e.currency_code || "EUR", n = e.total_holdings_precise ?? e.total_holdings, r = An(n), a = e.last_price_native ?? e.last_price?.native ?? e.last_price_eur, i = ye(a), o = i === "—" ? null : `${i}${`&nbsp;${k(t)}`}`, c = H(e.market_value_eur) ?? H(e.current_value_eur) ?? null, s = Ie(e.average_cost), l = s?.native ?? s?.security ?? null, f = s?.eur ?? null, p = s?.account ?? null ?? f, u = Ae(e.performance), g = u?.day_change ?? null, y = g?.price_change_native ?? null, h = g?.price_change_eur ?? null, m = R(y) ? y : h, b = R(y) ? t : "EUR", _ = (T, z = "") => {
    const B = ["value"];
    ...
    if (!R(T))
      return v(z);
    const B = ["value--gain"];
    return z && B.push(z), _(_i(T), B.join(" "));
  }, C = (T, z = "") => {
    if (!R(T))
      return v(z);
    const B = ["value--gain-percentage"];
    return z && B.push(z), _(bi(T), B.join(" "));
  }, P = o ? _(o, "value--price") : v("value--price"), N = r === "—" ? v("value--holdings") : _(r, "value--holdings"), F = R(c) ? _(`${ae(c)}&nbsp;€`, "value--market-value") : v("value--market-value"), I = R(m) ? _(
    bs(m, b),
  ...
  let L = null, j = null;
  return D && (t !== "EUR" || !R(l) || !Ke(f, l)) ? (L = f, j = "EUR") : R(p) && E && (E !== t || !Ke(p, l ?? NaN)) && (L = p, j = E), L != null && R(L) && S.push(
    _(
      `${ye(L)}${j ? `&nbsp;${k(j)}` : ""}`,
...
function ks(e, t, {
  ...
  return {
    ...
    markerTooltipRenderer: ({
      ...
      const m = g.payload ?? {}, b = pa(m.type), _ = H(m.shares), v = _ != null ? An(_) : null, A = Ne(m.currency) ?? s, C = [];
      b && C.push(b), v && C.push(`${v} Stück`), y && C.push(`am ${y}`);
      const P = C.join(" ").trim() || (typeof g.label == "string" ? g.label : y), N = typeof h == "string" && h.trim() ? h.trim() : ye(m.price), F = N ? `${N}${A ? `&nbsp;${k(A)}` : ""}` : k(A);
      return `
...
function Fs(e, t, n = {}) {
  ...
  let a = er.get(e) ?? null;
  if (!a || !e.contains(a)) {
    e.innerHTML = "", a = ia(e, r), a && er.set(e, a);
    return;
  }
  bn(a, r);
}
...
function Ts(e) {
  ...
  setTimeout(() => {
    ...
    if (!l)
      return;
    const f = ca(a), d = la(a), p = Qn(i);
    Array.isArray(c) && s.status !== "error" && f.set(o, c), ls(a), Xn(a, o), tr(l, o);
    const g = Qt(
      c,
      i
    );
    ...
    const h = async (m) => {
      ...
      b && (b.disabled = !0, b.classList.add("loading"), b.innerHTML = vi());
      let _ = f.get(m) ?? null, v = d.get(m) ?? null, A = null, C = [];
      if (_)
        A = _.length ? { status: "loaded" } : { status: "empty" };
      else
        try {
          const x = yt(m), U = await ct(
            n,
            r,
            a,
            x
          ...
          _ = Jt(U.prices), v = bt(
            ...
            i?.currency_code,
            i
          ), f.set(m, _), v = Array.isArray(v) ? v : [], d.set(m, v), A = _.length ? { status: "loaded" } : { status: "empty" };
        } catch (x) {
          console.error("Range-Wechsel: Historie konnte nicht geladen werden", x), _ = [], v = [], A = {
            status: "error",
            message: _a(x) || "Die historischen Daten konnten aufgrund eines Fehlers nicht geladen werden."
          };
        }
      if (!Array.isArray(v))
        try {
      ...
      C = Qt(_, i), A.status !== "error" && (A = C.length ? { status: "loaded" } : { status: "empty" });
      const P = Sn(i), { priceChange: N, priceChangePct: F } = ha(
        C,
        P
      ), I = Array.isArray(v) ? v : [];
      Xn(a, m), tr(l, m), $s(
    ...
    l.addEventListener("click", (m) => {
      const b = m.target?.closest(".security-range-button");
      if (!b || b.disabled)
        return;
...
function Rs(e) {
  ...
  c(), setTimeout(() => {
    ...
    if (!s)
      return;
    const l = (d) => {
      const p = (i?.placeholder || Gn).trim() || Gn, u = (i?.prompt_template || "").trim(), g = (i?.link || "").trim() || as;
...
Ticker: ${d}` : `Ticker: ${d}`, link: g };
      ...
      try {
        const { body: u, link: g } = l(d), y = await ws(u);
        y ? s.textContent = "✅ Copied! Opening..." : console.warn("News-Prompt: Clipboard unavailable – prompt could not be copied"), y && await new Promise((h) => setTimeout(h, 800)), As(g), !i && !o && c();
      } catch (u) {
...
async function Ls(e, t, n, r) {
  ...
  try {
    ...
    console.error("renderSecurityDetail: Snapshot konnte nicht geladen werden", w), o = ga(w);
  }
  const c = i || a, s = !!(a && !i), l = (c?.source ?? "") === "cache";
  r && os(r, c ?? null);
  const f = c && (s || l) ? is({ fallbackUsed: s, flaggedAsCache: l }) : "", d = c?.name || "Wertpapierdetails", p = Et(d, "", { includeMeta: !1 });
  ...
  if (o)
    ...
    `;
  const g = fa(r), y = ca(r), h = la(r);
  let m = y.has(g) ? y.get(g) ?? null : null, b = { status: "empty" }, _ = h.has(g) ? h.get(g) ?? null : null;
  if (Array.isArray(m))
    b = m.length ? { status: "loaded" } : { status: "empty" };
  else {
    m = [];
    try {
      const w = yt(g), x = await ct(
        t,
        n,
        r,
        w
      ...
      m = Jt(x.prices), _ = bt(
        ...
        c?.currency_code,
        c
      ), y.set(g, m), _ = Array.isArray(_) ? _ : [], h.set(g, _), b = m.length ? { status: "loaded" } : { status: "empty" };
    } catch (w) {
      console.error(
        ...
        status: "error",
        message: _a(w) || "Die historischen Daten konnten aufgrund eines Fehlers nicht geladen werden."
      };
    }
  }
  if (!Array.isArray(_))
    try {
      ...
      _ = bt(
        ...
        c?.currency_code,
        c
      ), y.set(g, U), _ = Array.isArray(_) ? _ : [], h.set(g, _), m = U, b = m.length ? { status: "loaded" } : { status: "empty" };
    } catch (w) {
      console.error(
  ...
  const A = vs(c, r), C = Ss(A), P = Sn(c), { priceChange: N, priceChangePct: F } = ha(
    v,
    P
  ), I = ma(
    g,
...
class Hs {
  ...
  renderCalendar(t, n) {
    ...
    for (let y = 1; y <= u.getDate(); y++) {
      ...
      const b = h.toLocaleDateString("de-DE", { weekday: "long", day: "numeric", month: "long", year: "numeric" });
      m.setAttribute("aria-label", b), this.applyDayClasses(m, h), m.addEventListener("click", (_) => {
        _.stopPropagation(), this.handleDayClick(h);
      }), m.addEventListener("keydown", (_) => {
        (_.key === "Enter" || _.key === " ") && (_.preventDefault(), _.stopPropagation(), this.handleDayClick(h));
...
function zs(e) {
  if (!e || typeof e != "object")
    return null;
  const t = e, n = en(t.start), r = en(t.end);
...
function Vs(e) {
  const t = en(e.date ?? null), n = zs(e.range ?? null);
  if (t && n)
...
function tn() {
  return {
    status: ve.status,
    error: ve.error,
    lastUpdated: ve.lastUpdated,
    data: Bs(ve.data),
    selection: Os(ve.selection)
  };
}
async function Ys(e, t, n = {}) {
  const r = Vs(n), a = Us(r);
  if (ve.data && !n.force && rr === a)
    return tn();
  qt({
    status: "loading",
    error: null,
...
function Gs() {
  ...
  return t.setUTCDate(e.getUTCDate() - (js - 1)), {
    range: {
      start: vt(t),
      end: vt(e)
    },
...
function or(e, t, n) {
  ...
  r.innerHTML = va(o.total_wealth_eur), a.innerHTML = Xs(n);
}
function sr(e, t) {
  ...
  const a = (o, c, s = "", l = "") => `
    <div class="metric-row ${s}" ${l ? `id="${l}"` : ""}>
      ...
      <span class="metric-value">${typeof c == "number" ? va(c) : c}</span>
    </div>`, i = `
    ...
    const d = Array.from(f.values()).map((p) => {
      ...
      return `
          <label class="scope-option">
            <input type="checkbox" data-scope-key="${me(u)}" ${g}>
            <span>${y}</span>
          </label>
        `;
    }).join("");
...
function Sa(e, t) {
  ...
  let c = o;
  !o.__chartState || !n.querySelector("svg") ? (n.innerHTML = "", c = ia(n, i)) : (bn(o, i), c = o), c && tc(c, r);
}
function nc(e) {
  if (!e.length)
    return null;
...
async function cr(e, t, n, r, a) {
  nt(e, "loading");
  const i = await Ys(n, r, a);
  if (i.status === "error") {
    if (nt(e, "error", i.error ?? void 0), t) {
  ...
  if (!o || !Array.isArray(o.records) || o.records.length === 0) {
    ...
    if (or(e, f, []), sr(e, []), nt(e, "loaded", "Keine Daten für den gewählten Zeitraum."), t) {
      const d = t.querySelector(".line-chart-container");
      d && d.replaceChildren();
    }
    return;
  }
  ba = a, nn = o, Zs(o.slices);
  const c = a.range ? `Zeitraum: ${a.range.start} – ${a.range.end}` : a.date ? `Tag: ${a.date}` : "";
  or(e, c, o.records), sr(e, o.records), t && (Js(t, o.slices), Sa(t, o)), nt(e, "loaded");
}
function rc(e, t, n, r) {
  const a = e.querySelector("#analyse-date-picker-container"), i = ba ?? tn().selection ?? Gs();
  let o;
  if (i.range)
    o = {
...
function ic(e) {
  ...
  const t = [
    ...
    const a = r.current_price ?? 0, i = r.last_sell_price ?? 0, o = a - i, c = o > 0 ? "positive" : o < 0 ? "negative" : "neutral";
    let s = k(r.name);
    return r.lots.length > 1 && (s = `
      ...
      last_sell_price: ge(r.last_sell_price),
      current_price: `<span class="trend--${c}">${ge(r.current_price)}</span>`,
      purchase_value_gross: ge(r.purchase_value_gross),
      sales_value_gross: ge(r.sales_value_gross),
      sales_value_net: ge(r.sales_value_net),
      result_pct: Ar(r.result_pct, Aa(r.result_pct / 100)),
      current_holdings: `${wa(r.current_holdings)} ${r.current_holdings === 0 ? '<ha-icon icon="mdi:lock-outline" title="Geschlossen"></ha-icon>' : ""}`
    };
  });
  return Ee(n, t, [], {
    sortable: !0,
...
function oc(e) {
  e.querySelectorAll(".expand-icon").forEach((t) => {
    t.addEventListener("click", (n) => {
      n.stopPropagation();
      const r = n.currentTarget.dataset.securityUuid, a = e.querySelector(`tr[data-security-uuid="${r}"]`), i = a?._row?._lots;
      if (!a || !i) return;
      const o = a.querySelector(".expand-icon ha-icon");
      if (a.classList.toggle("is-expanded")) {
        o?.setAttribute("icon", "mdi:chevron-down");
        const c = i.map((s) => {
          const l = document.createElement("tr");
          return l.classList.add("child-row"), l.dataset.parentUuid = r, l.innerHTML = `
            <td class="cell--name">
              <span class="child-indicator"></span>
              ${k(s.date)}
            </td>
            <td>${wa(s.shares)} Stk.</td>
            <td class="cell--right">${ge(s.sell_price)}</td>
            <td></td>
            <td class="cell--right">${ge(s.purchase_value_gross)}</td>
            <td class="cell--right">${ge(s.sales_value_gross)}</td>
            <td class="cell--right">${ge(s.sales_value_net)}</td>
            <td class="cell--right">${Ar(s.result_pct, Aa(s.result_pct / 100))}</td>
            <td></td>
          `, l;
        });
        a.after(...c);
      } else
        o?.setAttribute("icon", "mdi:chevron-right"), e.querySelectorAll(`tr.child-row[data-parent-uuid="${r}"]`).forEach((c) => c.remove());
    });
  });
}
async function sc(e, t, n) {
  const r = Et("Realisierte Performance", ""), a = await hi(t, n), i = ic(a), o = `
    ${r.outerHTML}
    <div class="card">
  ...
  `;
  return setTimeout(() => oc(e), 0), o;
}
const cc = _o, rn = "pp-reader-sticky-anchor", wt = "overview", lc = "trades", uc = "analyse", an = "security:", dc = [
  { key: wt, title: "Dashboard", render: jr },
  { key: lc, title: "Trades", render: sc },
  { key: uc, title: "Zeitmaschine", render: ac }
], He = /* @__PURE__ */ new Map(), et = [], At = /* @__PURE__ */ new Map();
let on = null, Wt = !1, Re = null, W = 0, Bt = null;
function Ct(e) {
  return typeof e == "object" && e !== null;
}
function Ca(e) {
...
function Cn(e) {
  return typeof e != "string" || !e.startsWith(an) ? null : e.slice(an.length) || null;
}
function hc() {
  if (!Re)
    return !1;
...
function de() {
  const e = et.map((t) => He.get(t)).filter((t) => !!t);
  return [...dc, ...e];
}
function mc(e) {
  const t = de();
...
async function yc(e, t, n, r) {
  ...
  if (c) {
    ...
    if (l && l.key === wt && wc(c, { suppressRender: !0 })) {
      const p = de().findIndex((u) => u.key === wt);
      s = p >= 0 ? p : 0;
    }
  }
  if (!Wt) {
...
function _c(e, t) {
  ...
  He.set(e, r), n && At.set(n, e), et.includes(e) || et.push(e);
}
function xa(e) {
  if (!e)
    return;
...
function ka(e) {
  ...
  const t = Da(e);
  let n = dr(t);
  if (!n && typeof on == "function")
    try {
      const i = on(e);
      i && typeof i.render == "function" ? (_c(t, i), n = dr(t)) : console.error("openSecurityDetail: Factory lieferte ungültigen Descriptor", i);
    } catch (i) {
  ...
  return a === -1 && (a = de().findIndex((o) => o.key === t), a === -1) ? (console.error("openSecurityDetail: Tab nach Registrierung nicht auffindbar"), !1) : (W = a, Re = null, sn(), !0);
}
function wc(e, t = {}) {
  if (!e)
    return console.error("closeSecurityDetail: Ungültige securityUuid", e), !1;
  const { suppressRender: n = !1 } = t, r = Da(e);
  if (!bc(r))
    return !1;
  ...
  if (!c.length)
    return W = 0, n || sn(), !0;
  if (Re = e, o) {
    const s = c.findIndex((l) => l.key === wt);
...
async function Fa(e, t, n) {
  ...
  try {
    ...
    console.error("renderTab: Fehler beim Rendern des Tabs:", f), e.innerHTML = `<div class="card"><h2>Fehler</h2><pre>${Pa(f)}</pre></div>`;
    return;
  }
  e.innerHTML = o ?? "", i.render === jr && _n(e);
  ...
  let l = e.querySelector(`#${rn}`);
  if (!l) {
    l = document.createElement("div"), l.id = rn;
    const f = s.parentNode;
    f && "insertBefore" in f && f.insertBefore(l, s);
  }
  Pc(e, t, n), Cc(e, t, n), Ac(e);
}
function Ac(e) {
  const t = e.querySelector(".header-card"), n = e.querySelector(`#${rn}`);
  if (!t || !n) {
...
class Nc extends HTMLElement {
  ...
  _handleBusEvent(t) {
    ...
    if (!n)
      return;
    const r = t.data;
    if (!fc(r.data_type) || r.entry_id && r.entry_id !== n)
      return;
    const a = gc(r.data_type, r.data);
  ...
  _queueUpdate(t, n) {
    ...
    t === "portfolio_positions" && (a.portfolioUuid = pc(
      r
    ));
    let i = -1;
...
customElements.get("pp-reader-dashboard") || customElements.define("pp-reader-dashboard", Nc);
console.log("PPReader dashboard module v20250914b geladen");
Ms({
  setSecurityDetailTabFactory: vc
});
export {
  Rc as __TEST_ONLY_DASHBOARD,
  Tc as __TEST_ONLY__,
  wc as closeSecurityDetail,
  yn as flushPendingPositions,
  dr as getDetailTabDescriptor,
  ho as handlePortfolioPositionsUpdate,
  bc as hasDetailTab,
  ka as openSecurityDetail,
  $c as reapplyPositionsSort,
  xc as registerDashboardElement,
  _c as registerDetailTab,
  kc as registerPanelHost,
  vc as setSecurityDetailTabFactory,
  Dc as unregisterDashboardElement,
  xa as unregisterDetailTab,
  Fc as unregisterPanelHost,
  Yr as updatePortfolioFooterFromDom
};
//# sourceMappingURL=dashboard.BU1sGgYL.js.map
