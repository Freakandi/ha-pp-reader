...
async function li(e, t, n) {
  ...
  const g = await e.connection.sendMessagePromise(u), m = si(g.range, f, p), y = (Array.isArray(g.records) ? g.records : []).map((b) => b && typeof b == "object" ? vr(b) : null).filter((b) => !!b), _ = ci(g.slices);
  return {
    range: m,
    records: y,
    ..._ ? { slices: _ } : {}
...
function Re(e, t, n = [], r = {}) {
  ...
  if (u != null && (g = `${ae(u)} %`, u > 0 ? m = "positive" : u < 0 && (m = "negative")), s += '<tr class="footer-row">', t.forEach((h, y) => {
    ...
    if (l[h.key] != null) {
      let v = "";
      h.key === "gain_abs" && g && (v = ` data-gain-pct="${Ee(g)}" data-gain-sign="${Ee(m)}"`), s += `<td${_}${v}>${L(h.key, l[h.key], void 0, d[h.key])}</td>`;
      return;
    }
    if (h.key === "gain_pct" && l.gain_pct != null) {
      s += `<td${_}>${L("gain_pct", l.gain_pct, void 0, d[h.key])}</td>`;
      return;
    }
    const b = d[h.key] ?? { hasValue: !1 };
    s += `<td${_}>${L(h.key, null, void 0, b)}</td>`;
...
function wi(e) {
  ...
  if (f)
    ...
    const b = U(t.gain_abs), v = U(t.gain_pct);
...
function Li(e) {
  if (!e)
    return null;
  const t = ee(e.uuid) ?? `${ki}-${e.name ?? "0"}`, n = Dr(e.name, "Unbenanntes Konto"), r = ee(e.currency_code), a = Z(e.balance), i = Z(e.orig_balance), o = "coverage_ratio" in e ? Fr(Z(e.coverage_ratio)) : null, c = ee(e.provenance), s = ee(e.metric_run_uuid), l = e.fx_unavailable === !0, d = Z(e.fx_rate), f = ee(e.fx_rate_source), p = ee(e.fx_rate_timestamp), u = [], g = $r(c);
  g && u.push(g);
  const m = {
    uuid: t,
    name: n,
...
function Mi(e) {
  ...
  if (!t)
    return null;
  const n = Dr(e.name, "Unbenanntes Depot"), r = Rn(e.position_count), a = Rn(e.missing_value_positions), i = Z(e.current_value), o = Z(e.purchase_sum) ?? Z(e.purchase_value_eur) ?? Z(e.purchase_value) ?? 0, c = Z(e.day_change_abs) ?? null, s = Z(e.day_change_pct) ?? null, l = Se(e.performance), d = l?.gain_abs ?? null, f = l?.gain_pct ?? null, p = l?.day_change ?? null;
  ...
  const m = i != null, h = e.has_current_value === !1 || !m, y = "coverage_ratio" in e ? Fr(Z(e.coverage_ratio)) : null, _ = ee(e.provenance), b = ee(e.metric_run_uuid), v = [], w = $r(_);
  w && v.push(w);
  const P = {
    uuid: t,
    name: n,
...
function Tr(e, t = {}) {
  ...
  const n = ["meta-badges", t.containerClass].filter(Boolean).join(" "), r = e.map((a) => {
    const i = `meta-badge--${a.tone}`, o = a.description ? ` title="${re(a.description)}"` : "";
    return `<span class="meta-badge ${i}"${o}>${re(
      a.label
    )}</span>`;
  }).join("");
...
function ut(e, t, n = {}) {
  ...
  return `<span class="${["name-with-badges", n.containerClass].filter(Boolean).join(" ")}"><span class="${a}">${re(
    e
  )}</span>${r}</span>`;
}
function Lr(e, t, n, r) {
...
function ao(e, t) {
  ...
  for (const [f, p] of d.entries()) {
    ...
    const j = typeof p.missing_value_positions == "number" && Number.isFinite(p.missing_value_positions) ? p.missing_value_positions : 0, S = P !== null, D = p.has_current_value === !1 || j > 0 || !S, T = at(h.textContent);
    ...
    const B = L("current_value", $.current_value, $, I), G = typeof P == "number" ? P : 0;
    if ((Math.abs(T - G) >= 5e-3 || h.innerHTML !== B) && (h.innerHTML = B, u.classList.add("flash-update"), setTimeout(() => {
      u.classList.remove("flash-update");
    }, 800)), y && (y.innerHTML = L("day_change_abs", N, $, I)), _ && (_.innerHTML = L("day_change_pct", k, $, I)), b) {
      const K = L("gain_abs", E, $, I);
      b.innerHTML = K;
  ...
  try {
    const f = (...y) => {
      ...
      return (b.length ? Array.from(b) : Array.from(y.querySelectorAll("tbody tr:not(.footer-row)"))).map((w) => {
        const P = _ ? w.cells.item(2) : w.cells.item(1);
      ...
      const _ = y.dataset.currentValue, b = y.dataset.purchaseSum, v = _ ? Number.parseFloat(_) : Number.NaN, w = b ? Number.parseFloat(b) : Number.NaN;
      return {
  ...
  try {
    ...
    if (o) {
      ...
      c.forEach((f, p) => {
        ...
        f.setAttribute("aria-label", `${re(g)} sortieren`);
...
function co(e) {
  ...
  const n = Array.from(e.querySelectorAll("tbody tr.portfolio-row")), r = (y) => {
    ...
    (y, _) => {
      ...
      y.valueRows += 1;
      const v = r(_.dataset.currentValue), w = r(_.dataset.gainAbs), P = r(_.dataset.purchaseSum);
...
function ho(e) {
  ...
  const w = b.join("<br>"), P = Ue(r?.purchase_value_eur) ?? 0, A = v.join(", ");
...
function Ge(e) {
  ...
  const t = [
    ...
    return {
      name: typeof a.name == "string" ? re(a.name) : typeof a.name == "number" ? String(a.name) : "",
  ...
  try {
    ...
    if (i) {
      ...
      return t.forEach((s, l) => {
        ...
        d.setAttribute("aria-label", `${re(f)} sortieren`);
...
function Wr(e) {
  ...
  const t = (S) => S == null || typeof S != "string" && typeof S != "number" && typeof S != "boolean" ? "" : re(S);
  ...
  r.forEach((S) => {
    ...
    const D = Number.isFinite(S.position_count) ? S.position_count : 0, T = Number.isFinite(S.purchase_sum) ? S.purchase_sum : 0, Y = S.hasValue && typeof S.current_value == "number" && Number.isFinite(S.current_value) ? S.current_value : null, $ = Y !== null, I = S.performance, W = typeof S.gain_abs == "number" ? S.gain_abs : typeof I?.gain_abs == "number" ? I.gain_abs : null, B = typeof S.gain_pct == "number" ? S.gain_pct : typeof I?.gain_pct == "number" ? I.gain_pct : null, G = I && typeof I == "object" ? I.day_change : null, K = typeof S.day_change_abs == "number" ? S.day_change_abs : G && typeof G == "object" ? G.value_change_eur ?? G.price_change_eur : null, ze = typeof S.day_change_pct == "number" ? S.day_change_pct : G && typeof G == "object" && typeof G.change_pct == "number" ? G.change_pct : null, Ce = S.fx_unavailable && $, Ea = typeof S.coverage_ratio == "number" && Number.isFinite(S.coverage_ratio) ? S.coverage_ratio : "", xa = typeof S.provenance == "string" ? S.provenance : "", Da = typeof S.metric_run_uuid == "string" ? S.metric_run_uuid : "", Ve = pt.has(S.uuid), Fa = Ve ? "portfolio-toggle expanded" : "portfolio-toggle", Pn = `portfolio-details-${S.uuid}`, J = {
      ...
      gain_abs: W,
      gain_pct: B
    }, Ae = { hasValue: $ }, ka = L("purchase_value", J.purchase_value, J, Ae), $a = L("current_value", J.current_value, J, Ae), Ra = L("day_change_abs", J.day_change_abs, J, Ae), Ta = L("day_change_pct", J.day_change_pct, J, Ae), La = L("gain_abs", J.gain_abs, J, Ae), Ma = L("gain_pct", J.gain_pct, J, Ae), An = $ && typeof B == "number" && Number.isFinite(B) ? `${ae(B)} %` : "", Ha = $ && typeof B == "number" && Number.isFinite(B) ? B > 0 ? "positive" : B < 0 ? "negative" : "neutral" : "", Ia = $ && typeof Y == "number" && Number.isFinite(Y) ? Y : "", za = $ && typeof W == "number" && Number.isFinite(W) ? W : "", Va = $ && typeof B == "number" && Number.isFinite(B) ? B : "", Ua = $ && typeof K == "number" && Number.isFinite(K) ? K : "", qa = $ && typeof ze == "number" && Number.isFinite(ze) ? ze : "", Wa = String(D);
    let Dt = "";
    ...
    const Oa = re(S.name), Ba = Tr(Ur(S.badges), {
  ...
  const a = e.filter((S) => typeof S.current_value == "number" && Number.isFinite(S.current_value)), i = e.reduce((S, D) => S + (Number.isFinite(D.position_count) ? D.position_count : 0), 0), o = a.reduce((S, D) => typeof D.current_value == "number" && Number.isFinite(D.current_value) ? S + D.current_value : S, 0), c = a.reduce((S, D) => typeof D.purchase_sum == "number" && Number.isFinite(D.purchase_sum) ? S + D.purchase_sum : S, 0), s = a.map((S) => {
    ...
    gain_abs: f ? d : null,
    gain_pct: f ? m : null
  }, y = { hasValue: f }, _ = { hasValue: u }, b = L("purchase_value", h.purchase_value, h, y), v = L("current_value", h.current_value, h, y), w = L("day_change_abs", h.day_change_abs, h, _), P = L("day_change_pct", h.day_change_pct, h, _), A = L("gain_abs", h.gain_abs, h, y), E = L("gain_pct", h.gain_pct, h, y);
  let F = "";
  ...
  const H = String(Math.round(i)), C = f ? String(o) : "", x = f ? String(c) : "", V = u ? String(l) : "", N = u && typeof g == "number" && Number.isFinite(g) ? String(g) : "", k = f ? String(d) : "", j = f && typeof m == "number" && Number.isFinite(m) ? String(m) : "";
...
function Or(e) {
  ...
  for (const T of r) {
    ...
    l = !0;
    const W = qe(T.dataset.currentValue), B = qe(T.dataset.gainAbs), G = qe(T.dataset.purchaseSum), K = qe(T.dataset.dayChange);
    if (W == null || B == null || G == null) {
      f = !1;
      continue;
    }
    i += W, c += B, o += G, K != null && (s += K, d = !0);
  }
  ...
  const y = Math.round(a).toLocaleString("de-DE"), _ = {
    ...
    gain_abs: u ? c : null,
    gain_pct: u ? g : null
  }, b = { hasValue: u }, v = { hasValue: d && u }, w = L("purchase_value", _.purchase_value, _, b), P = L("current_value", _.current_value, _, b), A = L("day_change_abs", _.day_change_abs, _, v), E = L("day_change_pct", _.day_change_pct, _, v), F = L("gain_abs", _.gain_abs, _, b), H = L("gain_pct", _.gain_pct, _, b), C = t.tHead ? t.tHead.rows.item(0) : null, x = C ? C.cells.length : 0, V = h.cells.length, N = x || V, k = N > 0 ? N <= 5 : !1, j = u && typeof g == "number" ? `${ae(g)} %` : "", S = u && typeof g == "number" ? g > 0 ? "positive" : g < 0 ? "negative" : "neutral" : "neutral";
  k ? h.innerHTML = `
      <td>Summe</td>
  ...
  D && (D.dataset.gainPct = j || "—", D.dataset.gainSign = S), h.dataset.positionCount = String(Math.round(a)), h.dataset.currentValue = u ? String(i) : "", h.dataset.purchaseSum = u ? String(o) : "", h.dataset.dayChange = u && d ? String(s) : "", h.dataset.dayChangePct = u && d && typeof m == "number" ? String(m) : "", h.dataset.gainAbs = u ? String(c) : "", h.dataset.gainPct = u && typeof g == "number" ? String(g) : "", h.dataset.hasValue = u ? "true" : "false", h.dataset.fxUnavailable = p ? "true" : "false";
}
function Ze(e, t) {
  if (!t) return;
  const n = e.querySelector(
...
async function Br(e, t, n) {
  ...
  `, _ = un("Übersicht", y), b = Wr(o), v = a.filter((C) => (C.currency_code ?? "EUR") === "EUR"), w = a.filter((C) => (C.currency_code ?? "EUR") !== "EUR"), A = w.some((C) => C.fx_unavailable) ? `
...
function Lt(e) {
  ...
  const { minY: c, maxY: s, boundedHeight: l } = r, d = Number.isFinite(c) ? c : o, p = (Number.isFinite(s) ? s : d + 1) - d, u = p === 0 ? 0.5 : (o - d) / p, g = ne(u, 0, 1), m = Math.max(l, 0), h = a.top + (1 - g) * m, y = Math.max(i - a.left - a.right, 0), _ = a.left, b = a.left + y;
...
function ko(e, t, n) {
  ...
  const s = e.map((N, k) => {
    const j = o(N, k), S = c(N, k), D = Kr(j, k), T = gt(S, Number.NaN);
  ...
  if (s.length === 0)
    return { points: [], range: null };
  const l = s.reduce((N, k) => Math.min(N, k.xValue), s[0].xValue), d = s.reduce((N, k) => Math.max(N, k.xValue), s[0].xValue), f = s.reduce((N, k) => Math.min(N, k.yValue), s[0].yValue), p = s.reduce((N, k) => Math.max(N, k.yValue), s[0].yValue), u = Math.max(r - i.left - i.right, 1), g = Math.max(a - i.top - i.bottom, 1), m = Number.isFinite(l) ? l : 0, h = Number.isFinite(d) ? d : m + 1, y = Number.isFinite(f) ? f : 0, _ = Number.isFinite(p) ? p : y + 1, b = gt(t.baseline?.value, null), v = b != null && Number.isFinite(b) ? Math.min(y, b) : y, w = b != null && Number.isFinite(b) ? Math.max(_, b) : _, P = Math.max(
    2,
    Math.min(
      6,
      Math.round(
    ...
    w,
    P
  ), F = Number.isFinite(A) ? A : y, H = Number.isFinite(E) ? E : _, C = h - m || 1, x = H - F || 1;
  return {
    points: s.map((N) => {
      const k = C === 0 ? 0.5 : (N.xValue - m) / C, j = x === 0 ? 0.5 : (N.yValue - F) / x, S = i.left + k * u, D = i.top + (1 - j) * g;
      return {
        ...N,
        x: S,
        y: D
...
function Mt(e) {
  ...
  r.forEach((l, d) => {
    ...
    if (!Number.isFinite(f) || !Number.isFinite(u))
      return;
    const g = c === 0 ? 0.5 : ne((f - a.minX) / c, 0, 1), m = s === 0 ? 0.5 : ne((u - a.minY) / s, 0, 1), h = i.left + g * a.boundedWidth, y = i.top + (1 - m) * a.boundedHeight, _ = oe("g", {
...
function Ro(e, t, n, r = null) {
  ...
  const f = a.offsetWidth || 0, p = a.offsetHeight || 0, u = t.x * s, g = ne(
    ...
    o.left * s,
    (i - o.right) * s - f
  ), m = Math.max(d * l - p, 0), h = 12, _ = (Number.isFinite(n) ? ne(n ?? 0, o.top, d) : t.y) * l;
  let b = _ - p - h;
  b < o.top * l && (b = _ + h), b = ne(b, 0, m);
  const v = Q(Math.round(g)), w = Q(Math.round(b));
...
function Lo(e, t, n, r = null) {
  ...
  const p = a.offsetWidth || 0, u = a.offsetHeight || 0, g = t.x * l, m = ne(
    ...
    o.left * l,
    (i - o.right) * l - p
  ), h = Math.max(f * d - u, 0), y = 10, _ = s?.getBoundingClientRect(), b = e.svg?.getBoundingClientRect(), v = _ && b ? _.top - b.top : null, w = _ && b ? _.bottom - b.top : null, A = (Number.isFinite(n) ? ne(n ?? t.y, o.top, f) : t.y) * d;
  let E;
  v != null && w != null ? v <= A ? E = v - u - y : E = w + y : (E = A - u - y, E < o.top * d && (E = A + y)), E = ne(E, 0, h);
...
function Ht(e) {
  ...
  if (t.style.left = Q(a.left), t.style.width = Q(m), t.style.top = Q(i - a.bottom + 6), t.innerHTML = "", u && m > 0) {
    const _ = (s - c) / Ao, b = Math.max(2, Math.min(6, Math.round(m / 140) || 4));
    zo(e, c, s, b, _).forEach(({ positionRatio: w, label: P }) => {
  ...
  if (n.style.left = "0", n.style.width = Q(Math.max(y, 0)), n.innerHTML = "", g && h > 0) {
    const _ = Math.max(2, Math.min(6, Math.round(h / 60) || 4)), b = Vo(l, d, _), v = o;
    b.forEach(({ value: w, positionRatio: P }) => {
      const A = document.createElement("div");
      A.className = "line-chart-axis-tick line-chart-axis-tick-y", A.style.position = "absolute", A.style.left = "0";
      const F = (1 - ne(P, 0, 1)) * h;
      A.style.top = Q(F), A.textContent = v(w, null, -1), n.appendChild(A);
    });
  }
}
function Io(e, t, n = 4) {
...
function _t(e, t, n = null) {
  ...
  return e.forEach((c, s) => {
    ...
    g && a && g !== a && se(o) && (u *= o);
    const h = M(c.shares), y = M(c.net_price_eur), _ = d ? "Kauf" : "Verkauf", b = h != null ? `${Cn(h)} @ ` : "", v = `${_} ${b}${ge(u)} ${m}`, w = f && y != null ? `${v} (netto ${ge(y)} EUR)` : v, P = d ? Go : Xo, A = typeof c.uuid == "string" && c.uuid.trim() || `${_}-${p.getTime().toString()}-${s.toString()}`;
    r.push({
      id: A,
      x: p.getTime(),
      y: u,
...
function ws(e, t, {
  ...
  return {
    ...
    markerTooltipRenderer: ({
      ...
      const y = g.payload ?? {}, _ = da(y.type), b = M(y.shares), v = b != null ? Cn(b) : null, w = Pe(y.currency) ?? s, P = [];
      _ && P.push(_), v && P.push(`${v} Stück`), m && P.push(`am ${m}`);
...
function Ns(e) {
  ...
  setTimeout(() => {
    ...
    const h = async (y) => {
      ...
      let b = d.get(y) ?? null, v = f.get(y) ?? null, w = null, P = [];
      if (b)
        ...
        try {
          ...
          b = Xt(V.prices), v = _t(
            ...
            i?.currency_code,
            i
          ), d.set(y, b), v = Array.isArray(v) ? v : [], f.set(y, v), w = b.length ? { status: "loaded" } : { status: "empty" };
        } catch (x) {
          console.error("Range-Wechsel: Historie konnte nicht geladen werden", x), b = [], v = [], w = {
            status: "error",
      ...
      P = Zt(b, i), w.status !== "error" && (w = P.length ? { status: "loaded" } : { status: "empty" });
  ...
  if (!Array.isArray(b))
    try {
      ...
      b = _t(
        ...
        c?.currency_code,
        c
      ), m.set(g, V), b = Array.isArray(b) ? b : [], h.set(g, b), y = V, _ = y.length ? { status: "loaded" } : { status: "empty" };
    } catch (C) {
      console.error(
...
class Fs {
  ...
  renderCalendar(t, n) {
    ...
    for (let m = 1; m <= u.getDate(); m++) {
      ...
      y.className = "drp-day", y.textContent = m.toString(), this.applyDayClasses(y, h), y.addEventListener("click", (_) => {
...
function Bs(e, t) {
  ...
  const o = (c, s, l) => {
    ...
    const f = Array.from(d.values()).map((p) => {
      ...
      if (!u)
        return "";
      const g = fe.has(u) ? "checked" : "", m = re(p.scope_name ?? p.scope_id);
      return `
...
//# sourceMappingURL=dashboard.DjwhFJAZ.js.map
