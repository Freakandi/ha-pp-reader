...
async function ui(e, t, n) {
  ...
  const { date: a, range: i, includeSlices: o, includeScopes: c, scopes: s, limit: l, offset: u } = n, f = ne(a), p = i && typeof i == "object" ? {
    start: ne(i.start) ?? "",
    end: ne(i.end) ?? ""
  } : null;
  ...
  const g = await e.connection.sendMessagePromise(d), m = ci(g.range, f, p), y = (Array.isArray(g.records) ? g.records : []).map((b) => b && typeof b == "object" ? vr(b) : null).filter((b) => !!b), _ = li(g.slices);
  return {
    range: m,
    records: y,
    ..._ ? { slices: _ } : {}
...
function Re(e, t, n = [], r = {}) {
  ...
  t.forEach((h) => {
    if (n.includes(h.key)) {
      ...
      y.hasValue ? (l[h.key] = y.total, u[h.key] = { hasValue: !0 }) : (l[h.key] = null, u[h.key] = { hasValue: !1 });
    }
  });
  ...
  if (p != null) {
    ...
    if (h != null) {
      const y = h - p;
      y && (l.day_change_pct = p / y * 100, u.day_change_pct = { hasValue: !0 });
    }
  }
  const d = Number.isFinite(l.gain_pct ?? NaN) ? l.gain_pct : null;
  let g = "", m = "neutral";
  if (d != null && (g = `${ae(d)} %`, d > 0 ? m = "positive" : d < 0 && (m = "negative")), s += '<tr class="footer-row">', t.forEach((h, y) => {
    ...
    if (l[h.key] != null) {
      let v = "";
      h.key === "gain_abs" && g && (v = ` data-gain-pct="${ge(g)}" data-gain-sign="${ge(m)}"`), s += `<td${_}${v}>${M(h.key, l[h.key], void 0, u[h.key])}</td>`;
      return;
    }
    if (h.key === "gain_pct" && l.gain_pct != null) {
      s += `<td${_}>${M("gain_pct", l.gain_pct, void 0, u[h.key])}</td>`;
      return;
    }
    const b = u[h.key] ?? { hasValue: !1 };
...
function Ar(e, t, n = "asc", r = !1) {
  ...
  o.sort((u, f) => {
    const p = u.cells.item(c), d = f.cells.item(c), g = (p?.textContent ?? "").trim(), m = (d?.textContent ?? "").trim(), h = s(g), y = s(m);
    let _;
  ...
  if (typeof e == "string") {
    ...
    if (c && (!s || a > i))
      if (s)
        ...
        const f = o.split(","), p = f[f.length - 1]?.length ?? 0, d = f.slice(0, -1).join(""), g = d.replace(/[+-]/g, "").length, m = f.length > 2, h = /^[-+]?0$/.test(d);
...
function Ae(e) {
  ...
  if (n == null || r == null || a == null || i == null)
    return null;
  const o = wr(t.source) ?? "derived", c = ce(t.coverage_ratio) ?? null, s = _i(t.day_change);
  return {
    gain_abs: n,
    gain_pct: r,
    total_change_eur: a,
...
function Hi(e) {
  if (!e)
    return null;
  const t = te(e.uuid) ?? `${Ri}-${e.name ?? "0"}`, n = Dr(e.name, "Unbenanntes Konto"), r = te(e.currency_code), a = J(e.balance), i = J(e.orig_balance), o = "coverage_ratio" in e ? Fr(J(e.coverage_ratio)) : null, c = te(e.provenance), s = te(e.metric_run_uuid), l = e.fx_unavailable === !0, u = J(e.fx_rate), f = te(e.fx_rate_source), p = te(e.fx_rate_timestamp), d = [], g = $r(c);
  g && d.push(g);
  const m = {
    uuid: t,
    name: n,
...
function Ii(e) {
  ...
  if (!t)
    return null;
  const n = Dr(e.name, "Unbenanntes Depot"), r = Rn(e.position_count), a = Rn(e.missing_value_positions), i = J(e.current_value), o = J(e.purchase_sum) ?? J(e.purchase_value_eur) ?? J(e.purchase_value) ?? 0, c = J(e.day_change_abs) ?? null, s = J(e.day_change_pct) ?? null, l = Ae(e.performance), u = l?.gain_abs ?? null, f = l?.gain_pct ?? null, p = l?.day_change ?? null;
  let d = c ?? (p?.value_change_eur != null ? J(p.value_change_eur) : null), g = s ?? (p?.change_pct != null ? J(p.change_pct) : null);
...
function ao(e, t) {
  ...
  if (r) {
    const o = i.map((c) => {
      const s = c.orig_balance, l = typeof s == "number" && Number.isFinite(s), u = Ie(c.currency_code), f = l ? s.toLocaleString("de-DE", {
...
function oo(e, t) {
  ...
  for (const [f, p] of u.entries()) {
    ...
    const g = d.cells.item(1), m = d.cells.item(2), h = d.cells.item(3), y = d.cells.item(4), _ = d.cells.item(5), b = d.cells.item(6), v = d.cells.item(7);
    if (!g || !m || !h)
      continue;
    const w = typeof p.position_count == "number" && Number.isFinite(p.position_count) ? p.position_count : 0, C = typeof p.current_value == "number" && Number.isFinite(p.current_value) ? p.current_value : null, P = Ae(p.performance), N = typeof P?.gain_abs == "number" ? P.gain_abs : null, k = typeof P?.gain_pct == "number" ? P.gain_pct : null, I = typeof p.purchase_sum == "number" && Number.isFinite(p.purchase_sum) ? p.purchase_sum : typeof p.purchase_value == "number" && Number.isFinite(p.purchase_value) ? p.purchase_value : null, A = P?.day_change ?? null, x = Ne(p.day_change_abs) ?? Ne(A?.value_change_eur) ?? Ne(A?.price_change_eur), U = Ne(p.day_change_pct) ?? Ne(A?.change_pct);
    let E = x ?? null, $ = U ?? null;
    ...
    const Y = M("current_value", R.current_value, R, z), X = typeof C == "number" ? C : 0;
    if ((Math.abs(L - X) >= 5e-3 || h.innerHTML !== Y) && (h.innerHTML = Y, d.classList.add("flash-update"), setTimeout(() => {
      ...
      b.innerHTML = G;
      const we = typeof k == "number" && Number.isFinite(k) ? k : null;
      b.dataset.gainPct = we != null ? `${i(we)} %` : "—", b.dataset.gainSign = we != null ? we > 0 ? "positive" : we < 0 ? "negative" : "neutral" : "neutral";
    }
    v && (v.innerHTML = M("gain_pct", k, R, z)), d.dataset.positionCount = w.toString(), d.dataset.purchaseSum = I != null ? I.toString() : "", d.dataset.currentValue = S ? X.toString() : "", d.dataset.dayChange = S && E != null ? E.toString() : "", d.dataset.dayChangePct = S && $ != null ? $.toString() : "", d.dataset.gainAbs = N != null ? N.toString() : "", d.dataset.gainPct = k != null ? k.toString() : "", d.dataset.hasValue = S ? "true" : "false", d.dataset.fxUnavailable = D ? "true" : "false", d.dataset.coverageRatio = typeof p.coverage_ratio == "number" && Number.isFinite(p.coverage_ratio) ? p.coverage_ratio.toString() : "", d.dataset.provenance = typeof p.provenance == "string" ? p.provenance : "", d.dataset.metricRunUuid = typeof p.metric_run_uuid == "string" ? p.metric_run_uuid : "", s += 1;
  }
  if (s === 0)
  ...
  try {
    const f = (...y) => {
      ...
      return (b.length ? Array.from(b) : Array.from(y.querySelectorAll("tbody tr:not(.footer-row)"))).map((w) => {
        const C = _ ? w.cells.item(2) : w.cells.item(1);
...
function In(e, t) {
  ...
  if (f.applied) {
    ...
    r || f.reason !== "hidden" || l ? (ue.set(n, { positions: u, error: r }), Ir(t, n)) : (ue.delete(n), Be.delete(n));
  if (!r && o.length > 0) {
    const p = Array.from(
      new Set(
  ...
  try {
    ...
    if (o) {
      ...
      c.forEach((f, p) => {
        const d = s[p];
        if (!d) return;
        f.setAttribute("data-sort-key", d), f.classList.add("sortable-col"), f.setAttribute("role", "button"), f.setAttribute("tabindex", "0"), f.setAttribute("aria-sort", "none");
      ...
      if (u)
        ...
        o.querySelectorAll("tbody tr").forEach((p, d) => {
          ...
          if (!g)
            return;
          const m = e[d], h = Ln(m), y = typeof h?.gain_pct == "number" && Number.isFinite(h.gain_pct) ? h.gain_pct : null, _ = y != null ? `${y.toLocaleString("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
...
function uo(e) {
  ...
  const n = Array.from(e.querySelectorAll("tbody tr.portfolio-row")), r = (y) => {
    ...
    (y, _) => {
      ...
      y.valueRows += 1;
      const v = r(_.dataset.currentValue), w = r(_.dataset.gainAbs), C = r(_.dataset.purchaseSum);
      return v == null || w == null || C == null ? (y.incompleteRows += 1, y) : (y.sumCurrent += v, y.sumGainAbs += w, y.sumPurchase += C, y);
    },
    {
      sumCurrent: 0,
      sumGainAbs: 0,
  ...
  }, u = { hasValue: i }, f = M("current_value", l.current_value, l, u), p = i ? a.sumGainAbs : null, d = i ? o : null, g = M("gain_abs", p, l, u), m = M("gain_pct", d, l, u);
  c.innerHTML = `
    <td>Summe</td>
...
function Vr(e, t, n) {
  const r = n ?? document, i = (Array.isArray(e) ? e : []).reduce((f, p) => {
    const d = p.balance ?? p.current_value ?? p.value, g = zn(d);
    return f + g;
  }, 0), c = (Array.isArray(t) ? t : []).reduce((f, p) => {
    const d = p.current_value ?? p.value, g = zn(d);
    return f + g;
  ...
  u ? u.textContent = `${Bt(s)} €` : l.textContent = `💰 Gesamtvermögen: ${Bt(s)} €`, l.dataset.totalWealthEur = s.toString();
}
function fo(e, t) {
...
function yo(e) {
  const t = e, n = e.average_cost ?? null, r = e.aggregation ?? null, a = On(t, [
    ...
    a === "EUR" ? "EUR" : null
  ) ?? "EUR", o = Ue(n?.native), c = Ue(n?.security), s = Ue(n?.account), l = Ue(n?.eur), u = c ?? o, f = l ?? (i === "EUR" ? s : null), p = a ?? i, d = p === "EUR";
  let g, m;
  d ? (g = "EUR", m = f ?? u ?? s ?? null) : u != null ? (g = p, m = u) : s != null ? (g = i, m = s) : (g = "EUR", m = f ?? null);
  const h = Bn(m, g), y = d ? null : Bn(f, "EUR"), _ = !!y && y !== h, b = [], v = [];
  h ? (b.push(
  ...
  const w = b.join("<br>"), C = Ue(r?.purchase_value_eur) ?? 0, P = v.join(", ");
...
function Ge(e) {
  ...
  const t = [
    ...
    const i = Ae(a.performance), o = typeof i?.gain_abs == "number" ? i.gain_abs : null, c = typeof i?.gain_pct == "number" ? i.gain_pct : null, s = bo(a), l = typeof a.purchase_value == "number" || typeof a.purchase_value == "string" ? a.purchase_value : null;
    return {
  ...
  try {
    ...
    if (i) {
      ...
      return t.forEach((s, l) => {
        ...
        if (!u)
          return;
        u.setAttribute("data-sort-key", s.key), u.classList.add("sortable-col"), u.setAttribute("role", "button"), u.setAttribute("tabindex", "0"), u.setAttribute("aria-sort", "none");
        const f = u.textContent || "";
        u.setAttribute("aria-label", `${F(f)} sortieren`);
        ...
        if (p) {
          const { markup: m, sortValue: h, ariaLabel: y } = yo(u);
        ...
        if (d) {
          const m = Ae(u.performance), h = typeof m?.gain_pct == "number" && Number.isFinite(m.gain_pct) ? m.gain_pct : null, y = h != null ? `${h.toLocaleString("de-DE", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2
          })} %` : "—", _ = h == null ? "neutral" : h > 0 ? "positive" : h < 0 ? "negative" : "neutral";
          d.dataset.gainPct = y, d.dataset.gainSign = _;
        }
...
function Wr(e) {
  ...
  r.forEach((S) => {
    ...
    let Dt = "";
    Pn && (Dt = ` data-gain-pct="${t(Pn)}" data-gain-sign="${t(Ia)}"`), we && (Dt += ' data-partial="true"'), n += `<tr class="portfolio-row"
    ...
    n += `<td class="align-right">${ja}</td>`, n += `<td class="align-right">${$a}</td>`, n += `<td class="align-right">${Ra}</td>`, n += `<td class="align-right">${Ta}</td>`, n += `<td class="align-right">${La}</td>`, n += `<td class="align-right"${Dt}>${Ma}</td>`, n += `<td class="align-right gain-pct-cell">${Ha}</td>`, n += "</tr>", n += `<tr class="portfolio-details${Ve ? "" : " hidden"}"
      ...
      <td colspan="${r.length.toString()}">
        <div class="positions-container">${Ve ? Et(S.uuid) ? Ge(Cr(S.uuid)) : dn("Lade Positionen...") : ""}</div>
      </td>
    </tr>`;
  });
  const a = e.filter((S) => typeof S.current_value == "number" && Number.isFinite(S.current_value)), i = e.reduce((S, D) => S + (Number.isFinite(D.position_count) ? D.position_count : 0), 0), o = a.reduce((S, D) => typeof D.current_value == "number" && Number.isFinite(D.current_value) ? S + D.current_value : S, 0), c = a.reduce((S, D) => typeof D.purchase_sum == "number" && Number.isFinite(D.purchase_sum) ? S + D.purchase_sum : S, 0), s = a.map((S) => {
    ...
    gain_abs: f ? u : null,
    gain_pct: f ? m : null
  }, y = { hasValue: f }, _ = { hasValue: d }, b = M("purchase_value", h.purchase_value, h, y), v = M("current_value", h.current_value, h, y), w = M("day_change_abs", h.day_change_abs, h, _), C = M("day_change_pct", h.day_change_pct, h, _), P = M("gain_abs", h.gain_abs, h, y), N = M("gain_pct", h.gain_pct, h, y);
  let k = "";
  ...
  const I = String(Math.round(i)), A = f ? String(o) : "", x = f ? String(c) : "", U = d ? String(l) : "", E = d && typeof g == "number" && Number.isFinite(g) ? String(g) : "", $ = f ? String(u) : "", K = f && typeof m == "number" && Number.isFinite(m) ? String(m) : "";
...
function Or(e) {
  ...
  const y = Math.round(a).toLocaleString("de-DE"), _ = {
    ...
    gain_abs: d ? c : null,
    gain_pct: d ? g : null
  }, b = { hasValue: d }, v = { hasValue: u && d }, w = M("purchase_value", _.purchase_value, _, b), C = M("current_value", _.current_value, _, b), P = M("day_change_abs", _.day_change_abs, _, v), N = M("day_change_pct", _.day_change_pct, _, v), k = M("gain_abs", _.gain_abs, _, b), I = M("gain_pct", _.gain_pct, _, b), A = t.tHead ? t.tHead.rows.item(0) : null, x = A ? A.cells.length : 0, U = h.cells.length, E = x || U, $ = E > 0 ? E <= 5 : !1, K = d && typeof g == "number" ? `${ae(g)} %` : "", S = d && typeof g == "number" ? g > 0 ? "positive" : g < 0 ? "negative" : "neutral" : "neutral";
  $ ? h.innerHTML = `
      <td>Summe</td>
  ...
  D && (D.dataset.gainPct = K || "—", D.dataset.gainSign = S), h.dataset.positionCount = String(Math.round(a)), h.dataset.currentValue = d ? String(i) : "", h.dataset.purchaseSum = d ? String(o) : "", h.dataset.dayChange = d && u ? String(s) : "", h.dataset.dayChangePct = d && u && typeof m == "number" ? String(m) : "", h.dataset.gainAbs = d ? String(c) : "", h.dataset.gainPct = d && typeof g == "number" ? String(g) : "", h.dataset.hasValue = d ? "true" : "false", h.dataset.fxUnavailable = p ? "true" : "false";
}
function Ze(e, t) {
  if (!t) return;
  const n = e.querySelector(
  ...
  const i = (d, g) => {
    ...
    h.sort((v, w) => {
      const P = {
        ...
        gain_abs: 7,
        gain_pct: 8
      }[d], N = v.cells.item(P), k = w.cells.item(P);
      let I = "";
      if (N) {
      ...
      let U;
      if (d === "name")
        U = I.localeCompare(A, "de", { sensitivity: "base" });
      else {
        const E = x(N, I), $ = x(k, A);
        U = E - $;
      }
  ...
  }, o = r.dataset.sortKey, c = r.dataset.sortDir, s = a.dataset.defaultSort, l = a.dataset.defaultDir, u = Rt(o) ? o : Rt(s) ? s : "name", f = Tt(c) ? c : Tt(l) ? l : "asc";
  i(u, f);
  const p = (d) => {
    const g = d.target;
        ...
        (async () => {
          try {
            ...
            if (u.classList.contains("hidden")) {
              u.classList.remove("hidden"), s.classList.add("expanded"), s.setAttribute("aria-expanded", "true"), f && (f.textContent = "▼"), pt.add(l);
              try {
                mn(e, l);
              } catch (d) {
              ...
              if (Et(l)) {
                ...
                const d = u.querySelector(".positions-container");
                d && (d.innerHTML = dn("Lade Positionen..."));
                try {
                  const g = await Sr(
                    dt,
                    ft,
                  ...
                  if (g.error) {
                    ...
                    d && (d.innerHTML = `<div class="error">${F(h)} <button class="retry-pos" data-portfolio="${l}">Erneut laden</button></div>`);
                    return;
                  }
                  const m = Nt(
                  ...
                  const m = g instanceof Error ? g.message : String(g), h = u.querySelector(".positions-container");
              ...
              u.classList.add("hidden"), s.classList.remove("expanded"), s.setAttribute("aria-expanded", "false"), f && (f.textContent = "▶"), pt.delete(l);
          } catch (o) {
...
async function Br(e, t, n) {
  ...
  const s = a.reduce(
    ...
    0
  ), l = o.some((A) => A.fx_unavailable), u = a.some((A) => A.fx_unavailable && (A.balance == null || !Number.isFinite(A.balance))), f = o.reduce((A, x) => x.hasValue && typeof x.current_value == "number" && Number.isFinite(x.current_value) ? A + x.current_value : A, 0), p = s + f, d = "Teilw. fehlende FX-Kurse – Gesamtvermögen abweichend", m = o.some((A) => A.hasValue && typeof A.current_value == "number" && Number.isFinite(A.current_value)) || a.some((A) => typeof A.balance == "number" && Number.isFinite(A.balance)) ? `${ae(p)}&nbsp;€` : `<span class="missing-value" role="note" aria-label="${d}" title="${d}">—</span>`, h = l || u ? `<span class="total-wealth-note">${d}</span>` : "", y = `
    <div class="header-meta-row">
      💰 Gesamtvermögen: <strong class="total-wealth-value">${m}</strong>${h}
    </div>
  `, _ = un("Übersicht", y), b = Wr(o), v = a.filter((A) => (A.currency_code ?? "EUR") === "EUR"), w = a.filter((A) => (A.currency_code ?? "EUR") !== "EUR"), P = w.some((A) => A.fx_unavailable) ? `
...
function Lt(e) {
  ...
  const { minY: c, maxY: s, boundedHeight: l } = r, u = Number.isFinite(c) ? c : o, p = (Number.isFinite(s) ? s : u + 1) - u, d = p === 0 ? 0.5 : (o - u) / p, g = re(d, 0, 1), m = Math.max(l, 0), h = a.top + (1 - g) * m, y = Math.max(i - a.left - a.right, 0), _ = a.left, b = a.left + y;
...
function Ro(e, t, n) {
  ...
  if (s.length === 0)
    return { points: [], range: null };
  const l = s.reduce((E, $) => Math.min(E, $.xValue), s[0].xValue), u = s.reduce((E, $) => Math.max(E, $.xValue), s[0].xValue), f = s.reduce((E, $) => Math.min(E, $.yValue), s[0].yValue), p = s.reduce((E, $) => Math.max(E, $.yValue), s[0].yValue), d = Math.max(r - i.left - i.right, 1), g = Math.max(a - i.top - i.bottom, 1), m = Number.isFinite(l) ? l : 0, h = Number.isFinite(u) ? u : m + 1, y = Number.isFinite(f) ? f : 0, _ = Number.isFinite(p) ? p : y + 1, b = gt(t.baseline?.value, null), v = b != null && Number.isFinite(b) ? Math.min(y, b) : y, w = b != null && Number.isFinite(b) ? Math.max(_, b) : _, C = Math.max(
    2,
    Math.min(
      6,
      Math.round(
    ...
    w,
    C
  ), k = Number.isFinite(P) ? P : y, I = Number.isFinite(N) ? N : _, A = h - m || 1, x = I - k || 1;
  return {
    points: s.map((E) => {
      const $ = A === 0 ? 0.5 : (E.xValue - m) / A, K = x === 0 ? 0.5 : (E.yValue - k) / x, S = i.left + $ * d, D = i.top + (1 - K) * g;
      return {
        ...E,
        x: S,
        y: D
...
function Mt(e) {
  ...
  r.forEach((l, u) => {
    const f = Kr(l.x, u), p = gt(l.y, Number.NaN), d = Number(p);
    if (!Number.isFinite(f) || !Number.isFinite(d))
      return;
    const g = c === 0 ? 0.5 : re((f - a.minX) / c, 0, 1), m = s === 0 ? 0.5 : re((d - a.minY) / s, 0, 1), h = i.left + g * a.boundedWidth, y = i.top + (1 - m) * a.boundedHeight, _ = oe("g", {
...
function Lo(e, t, n, r = null) {
  ...
  const f = a.offsetWidth || 0, p = a.offsetHeight || 0, d = t.x * s, g = re(
    d - f / 2,
    o.left * s,
    (i - o.right) * s - f
  ), m = Math.max(u * l - p, 0), h = 12, _ = (Number.isFinite(n) ? re(n ?? 0, o.top, u) : t.y) * l;
  let b = _ - p - h;
...
function Ho(e, t, n, r = null) {
  ...
  a.style.visibility = "visible", a.style.opacity = "1";
  const p = a.offsetWidth || 0, d = a.offsetHeight || 0, g = t.x * l, m = re(
    g - p / 2,
    o.left * l,
    (i - o.right) * l - p
  ), h = Math.max(f * u - d, 0), y = 10, _ = s?.getBoundingClientRect(), b = e.svg?.getBoundingClientRect(), v = _ && b ? _.top - b.top : null, w = _ && b ? _.bottom - b.top : null, P = (Number.isFinite(n) ? re(n ?? t.y, o.top, f) : t.y) * u;
  let N;
  v != null && w != null ? v <= P ? N = v - d - y : N = w + y : (N = P - d - y, N < o.top * u && (N = P + y)), N = re(N, 0, h);
...
function zo(e, t) {
  ...
  const n = (a) => {
    ...
    const i = t.svg.getBoundingClientRect(), o = t.width || xe, c = t.height || De, s = i.width && Number.isFinite(i.width) && Number.isFinite(o) && o > 0 ? i.width / o : 1, l = i.height && Number.isFinite(i.height) && Number.isFinite(c) && c > 0 ? i.height / c : 1, u = s > 0 ? 1 / s : 1, f = l > 0 ? 1 / l : 1, p = (a.clientX - i.left) * u, d = (a.clientY - i.top) * f, g = {
      scaleX: s,
      scaleY: l
    };
    ...
    ), t.focusLine.style.opacity = "1"), t.tooltip && (t.tooltip.innerHTML = To(t, m), Lo(t, m, d, g));
    const y = Io(t, p, d);
    y && t.markerTooltip ? (t.markerTooltip.innerHTML = Mo(t, y), Ho(t, y, d, g)) : it(t);
  }, r = () => {
    jt(t), it(t);
  };
...
function ra(e, t = {}) {
  ...
  const g = ta(n);
  if (g.svg = r, g.areaPath = a, g.linePath = o, g.baselineLine = i, g.focusLine = c, g.focusCircle = s, g.overlay = u, g.tooltip = f, g.markerOverlay = p, g.markerLayer = l, g.markerTooltip = d, g.xAccessor = t.xAccessor ?? Gr, g.yAccessor = t.yAccessor ?? Xr, g.xFormatter = t.xFormatter ?? Zr, g.yFormatter = t.yFormatter ?? Jr, g.tooltipRenderer = t.tooltipRenderer ?? Qr, g.markerTooltipRenderer = t.markerTooltipRenderer ?? ea, g.color = t.color ?? Oe, g.areaColor = t.areaColor ?? Yt, g.baseline = t.baseline ?? null, g.handlersAttached = !1, g.markers = Array.isArray(t.markers) ? t.markers.slice() : [], !g.xAxis) {
...
function bn(e, t = {}) {
  ...
  if (i.length === 1) {
    ...
    n.linePath.setAttribute("d", u), n.areaPath && n.areaPath.setAttribute("d", ""), n.focusCircle && (n.focusCircle.setAttribute("cx", s.x.toFixed(2)), n.focusCircle.setAttribute("cy", s.y.toFixed(2)), n.focusCircle.style.opacity = "1"), n.focusLine && (n.focusLine.style.opacity = "0"), Ht(n), Lt(n), Mt(n);
    return;
  }
  const c = ko(i);
...
function Ht(e) {
  ...
  const { minX: c, maxX: s, minY: l, maxY: u, boundedWidth: f, boundedHeight: p } = r, d = Number.isFinite(c) && Number.isFinite(s) && s >= c, g = Number.isFinite(l) && Number.isFinite(u) && u >= l, m = Math.max(f, 0), h = Math.max(p, 0);
  if (t.style.left = ee(a.left), t.style.width = ee(m), t.style.top = ee(i - a.bottom + 6), t.innerHTML = "", d && m > 0) {
    const _ = (s - c) / No, b = Math.max(2, Math.min(6, Math.round(m / 140) || 4));
    Uo(e, c, s, b, _).forEach(({ positionRatio: w, label: C }) => {
      const P = document.createElement("div");
      P.className = "line-chart-axis-tick line-chart-axis-tick-x", P.style.position = "absolute", P.style.bottom = "0";
      const N = re(w, 0, 1);
      P.style.left = ee(N * m);
      let k = "-50%", I = "center";
      N <= 1e-3 ? (k = "0", I = "left", P.style.marginLeft = "2px") : N >= 0.999 && (k = "-100%", I = "right", P.style.marginRight = "2px"), P.style.transform = `translateX(${k})`, P.style.textAlign = I, P.textContent = C, t.appendChild(P);
    });
  }
  ...
  if (n.style.left = "0", n.style.width = ee(Math.max(y, 0)), n.innerHTML = "", g && h > 0) {
    const _ = Math.max(2, Math.min(6, Math.round(h / 60) || 4)), b = qo(l, u, _), v = o;
    b.forEach(({ value: w, positionRatio: C }) => {
      ...
      const k = (1 - re(C, 0, 1)) * h;
      P.style.top = ee(k), P.textContent = v(w, null, -1), n.appendChild(P);
    });
  }
}
function Vo(e, t, n = 4) {
...
function bt(e, t, n = null) {
  ...
  return e.forEach((c, s) => {
    const l = typeof c.type == "number" ? c.type : Number(c.type), u = Go.has(l), f = Xo.has(l);
    if (!u && !f)
      return;
    const p = cs(c.date);
    let d = H(c.price);
    ...
    g && a && g !== a && se(o) && (d *= o);
    const h = H(c.shares), y = H(c.net_price_eur), _ = u ? "Kauf" : "Verkauf", b = h != null ? `${An(h)} @ ` : "", v = `${_} ${b}${he(d)} ${m}`, w = f && y != null ? `${v} (netto ${he(y)} EUR)` : v, C = u ? Zo : Jo, P = typeof c.uuid == "string" && c.uuid.trim() || `${_}-${p.getTime().toString()}-${s.toString()}`;
    r.push({
      id: P,
      x: p.getTime(),
      y: d,
...
function _s(e, t, n) {
  ...
  let f = null;
  if (T(l) && l !== 0 && T(u) && (f = u / l), r?.source === "eur_total")
    return "EUR";
  const d = r?.eur;
  if (T(d) && je(a, d))
    return "EUR";
...
function As(e, t) {
  ...
  if (s > 0) {
    const _ = 1 / s;
    Number.isFinite(_) && _ > 0 && (u = Zn(_));
  }
  const f = vs(e), p = Ss(f), d = [`FX-Kurs (Kauf): 1 ${n} = ${l} ${c}`];
  u && d.push(`1 ${c} = ${u} ${n}`);
...
function ws(e) {
  ...
  const t = e.currency_code || "EUR", n = e.total_holdings_precise ?? e.total_holdings, r = An(n), a = e.last_price_native ?? e.last_price?.native ?? e.last_price_eur, i = he(a), o = i === "—" ? null : `${i}${`&nbsp;${F(t)}`}`, c = H(e.market_value_eur) ?? H(e.current_value_eur) ?? null, s = He(e.average_cost), l = s?.native ?? s?.security ?? null, u = s?.eur ?? null, p = s?.account ?? null ?? u, d = Ae(e.performance), g = d?.day_change ?? null, m = g?.price_change_native ?? null, h = g?.price_change_eur ?? null, y = T(m) ? m : h, _ = T(m) ? t : "EUR", b = (R, z = "") => {
    const O = ["value"];
  ...
  let L = null, j = null;
  return D && (t !== "EUR" || !T(l) || !je(u, l)) ? (L = u, j = "EUR") : T(p) && E && (E !== t || !je(p, l ?? NaN)) && (L = p, j = E), L != null && T(L) && S.push(
    b(
      `${he(L)}${j ? `&nbsp;${F(j)}` : ""}`,
...
function Ps(e, t, {
  ...
  const i = e.clientWidth || e.offsetWidth || 0, o = i > 0 ? i : 640, c = Math.min(Math.max(Math.floor(o * 0.5), 240), 440), s = (n || "").toUpperCase() || "EUR", l = T(r) ? r : null, u = Math.max(48, Math.min(72, Math.round(o * 0.075))), f = Math.max(28, Math.min(56, Math.round(o * 0.05))), p = Math.max(40, Math.min(64, Math.round(c * 0.14)));
  return {
    width: o,
    height: c,
    margin: {
    ...
    markerTooltipRenderer: ({
      ...
      const y = g.payload ?? {}, _ = da(y.type), b = H(y.shares), v = b != null ? An(b) : null, w = Pe(y.currency) ?? s, C = [];
      _ && C.push(_), v && C.push(`${v} Stück`), m && C.push(`am ${m}`);
      const P = C.join(" ").trim() || (typeof g.label == "string" ? g.label : m), N = typeof h == "string" && h.trim() ? h.trim() : he(y.price), k = N ? `${N}${w ? `&nbsp;${F(w)}` : ""}` : F(w);
      return `
...
function xs(e) {
  ...
  setTimeout(() => {
    ...
    if (!l)
      return;
    const u = oa(a), f = sa(a), p = Jn(i);
    Array.isArray(c) && s.status !== "error" && u.set(o, c), as(a), Gn(a, o), er(l, o);
    const g = Zt(
      c,
      i
    );
    ...
    const h = async (y) => {
      ...
      let b = u.get(y) ?? null, v = f.get(y) ?? null, w = null, C = [];
      if (b)
        ...
        try {
          ...
          b = Xt(U.prices), v = bt(
            ...
            i?.currency_code,
            i
          ), u.set(y, b), v = Array.isArray(v) ? v : [], f.set(y, v), w = b.length ? { status: "loaded" } : { status: "empty" };
        } catch (x) {
      ...
      C = Zt(b, i), w.status !== "error" && (w = C.length ? { status: "loaded" } : { status: "empty" });
...
function Ds(e) {
  ...
  c(), setTimeout(() => {
    ...
    if (!s)
      return;
    const l = (f) => {
      const p = (i?.placeholder || Kn).trim() || Kn, d = (i?.prompt_template || "").trim(), g = (i?.link || "").trim() || Qo;
...
Ticker: ${f}` : `Ticker: ${f}`, link: g };
      ...
      try {
        const { body: d, link: g } = l(f), m = await ys(d);
...
async function Fs(e, t, n, r) {
  ...
  try {
    ...
    console.error("renderSecurityDetail: Snapshot konnte nicht geladen werden", A), o = fa(A);
  }
  const c = i || a, s = !!(a && !i), l = (c?.source ?? "") === "cache";
  r && ts(r, c ?? null);
  const u = c && (s || l) ? es({ fallbackUsed: s, flaggedAsCache: l }) : "", f = c?.name || "Wertpapierdetails", p = un(f, "", { includeMeta: !1 });
  ...
  if (Array.isArray(y))
    ...
    try {
      ...
      console.error(
        ...
        status: "error",
        message: ma(A) || "Die historischen Daten konnten aufgrund eines Fehlers nicht geladen werden."
      };
    }
  }
  if (!Array.isArray(b))
  ...
  const w = hs(c, r), C = ms(w), P = vn(c), { priceChange: N, priceChangePct: k } = pa(
    v,
    P
  ), I = ga(
    g,
...
class $s {
  ...
  renderCalendar(t, n) {
    ...
    u.className = "drp-days-header", ["Mo", "Di", "Mi", "Do", "Fr", "Sa", "So"].forEach((m) => {
      ...
      h.className = "drp-day-name", h.textContent = m, u.appendChild(h);
    }), i.appendChild(u);
    const f = document.createElement("div");
...
function js(e, t) {
  ...
  const o = (c, s, l) => {
    ...
    const f = Array.from(u.values()).map((p) => {
      ...
      if (!d)
        return "";
      const g = fe.has(d) ? "checked" : "", m = F(p.scope_name ?? p.scope_id);
      return `
          <label class="scope-option">
            <input type="checkbox" data-scope-key="${ge(d)}" ${g}>
            <span>${m}</span>
          </label>
        `;
    }).join("");
  ...
  r.querySelectorAll(".analyse-slice-series").forEach((o) => {
    ...
    const c = o.points.map((u, f) => {
      ...
      if (p == null || !Number.isFinite(u.value))
        return null;
      const d = a.maxX === a.minX ? 0.5 : (p - a.minX) / (a.maxX - a.minX), g = a.maxY === a.minY ? 0.5 : (u.value - a.minY) / (a.maxY - a.minY), m = i.left + d * a.boundedWidth, h = i.top + (1 - g) * a.boundedHeight;
...
function Zs(e) {
  if (!e.length)
    return null;
  const t = e[0]?.total_wealth_eur ?? 0, n = e[e.length - 1]?.total_wealth_eur ?? 0, r = ie(e, "dividends_eur"), a = ie(e, "interest_eur"), i = r + a, o = -Math.abs(ie(e, "fees_eur")), c = -Math.abs(ie(e, "taxes_eur")), s = ie(e, "inbound_transfers_eur") - ie(e, "outbound_transfers_eur"), l = ie(e, "performance_neutral_movements"), u = n - t - i - o - c - s - l, f = ie(e, "realized_gains_eur"), p = ie(e, "unrealized_price_gains_eur"), d = ie(e, "fx_gains_eur");
  return {
    startValue: t,
    endValue: n,
    marketGain: u,
...
], Me = /* @__PURE__ */ new Map(), Qe = [], At = /* @__PURE__ */ new Map();
...
async function cc(e, t, n, r) {
  ...
  wa();
  const o = W >= 0 && W < a.length ? a[W] : null, c = o ? wn(o.key) : null;
  let s = i;
  if (c) {
    const l = i >= 0 && i < a.length ? a[i] : null;
    if (l && l.key === St && pc(c, { suppressRender: !0 })) {
      const p = de().findIndex((d) => d.key === St);
      s = p >= 0 ? p : 0;
    }
  }
  if (!Ut) {
...
function lc(e, t) {
  ...
  Me.set(e, r), n && At.set(n, e), Qe.includes(e) || Qe.push(e);
}
function Ca(e) {
  if (!e)
    return;
...
export {
  ...
  Ca as unregisterDetailTab,
  Ac as unregisterPanelHost,
  Or as updatePortfolioFooterFromDom
};
//# sourceMappingURL=dashboard.B-aJU5Cz.js.map
