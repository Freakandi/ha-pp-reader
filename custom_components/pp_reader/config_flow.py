    ...
    async def async_step_user(self, user_input=None):
        ...
        if user_input is not None:
            ...
            if not os.path.exists(file_path):
                ...
                db_path = Path(DEFAULT_DB_DIR) / (Path(file_path).stem + ".db") if db_use_default else Path(db_custom_path)

                if not db_use_default and (not db_path.parent.exists() or not db_path.parent.is_dir()):
